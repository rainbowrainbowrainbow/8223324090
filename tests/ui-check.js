...
checkPage('booking-summary.html', (doc, html) => {
    ...
    check('Booking summary hero header uses transparent decorative assets, real logo, and right booking card',
        ...
        && fs.existsSync(path.join(ROOT, 'images', 'banquet-top.png'))
        && fs.existsSync(path.join(ROOT, 'images', 'banquet-corner.png'))
        && fs.existsSync(path.join(ROOT, 'images', 'banquet-logo-down.png'))
        && fs.existsSync(path.join(ROOT, 'images', 'park-logo.png'))
        && pageCss.includes('--summary-teal: #087c83')
        && pageCss.includes('--summary-gold: #c9962e')
        && pageCss.includes('.banquet-top-plate')
        && pageCss.includes('.banquet-corner-art')
        && pageCss.includes('.banquet-final-brand')
        && pageCss.includes('grid-template-columns: 13mm minmax(0, 1fr) minmax(43mm, 52mm)')
        && pageCss.includes('min-height: 31mm')
        && pageCss.includes('padding: 4mm 4.5mm 2.8mm')
        && pageCss.includes('border-left: 2px solid var(--summary-gold)')
        && brandLogoRule.includes('object-fit: contain')
        && pageCss.includes('background: rgba(255, 255, 255, 0.82)')
        && pageCss.includes('background: rgba(255, 255, 255, 0.76)')
        && pageCss.includes('border: 1px solid rgba(201, 150, 46, 0.45)')
