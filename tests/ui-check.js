...
checkPage('staff.html', (doc, html) => {
    ...
    check('Staff schedule keeps premium HR Pulse switcher and unified panel rhythm',
        ...
        && html.includes('js/staff-schedule-shell.js?v=0.80.152')
        && html.includes('js/hr-pulse-switcher.js?v=0.80.152')
...
check('Profile unified completion pulse exposes Today and last-36 History as one local drilldown', profileCode.includes('function renderCabinetCompletionPulse') && profileCode.includes('function renderCabinetCompletionDetails') && profileCode.includes('function renderCabinetCompletionTabs') && profileCode.includes('function renderCabinetCompletionTaskRow') && profileCode.includes('function showMoreCabinetCompletionDetails') && profileCode.includes('data-cabinet-completion-tab="today"') && profileCode.includes('data-cabinet-completion-tab="history"') && profileCode.includes('aria-controls="cabinetCompletionDetails"') && profileCode.includes('Останні 36') && profileCode.includes('completedTodayTasks') && profileCode.includes('completedHistory') && profileCode.includes('data-cabinet-task-action="open"') && profileCode.includes('completedDashboardVisibleCount') && !profileCode.includes('Історія на 36 записів тут не використовується') && !profileCode.includes('<details class="cabinet-completed-details">') && !profileCode.includes('data-cabinet-completed-day-divider') && htmlContains('services/taskCabinetProjection.js', 'completedHistoryLimit') && htmlContains('services/taskCabinetProjection.js', "COALESCE(t.status, 'todo') = 'done'") && htmlContains('services/taskCabinetProjection.js', 'completedHistoryOverflow'));
...
check('HR grouped IA keeps Pulse clean and vacancy workspace owns hiring surfaces',
    ...
    && htmlContains('hr.html', 'js/hr-pulse-switcher.js?v=0.80.152')
    ...
    && htmlContains('hr.html', 'js/staff-schedule-shell.js?v=0.80.152')
    && htmlContains('hr.html', 'js/staff-page.js?v=0.80.152')
...
check('HR Account Center uses the dedicated effective-access workspace',
    htmlContains('hr.html', 'css/account-access-editor.css?v=0.80.152')
    && htmlContains('hr.html', 'js/account-access-editor.js?v=0.80.152')
