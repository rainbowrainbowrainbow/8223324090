...
const GREETINGS = {
    ...
    evening: [
        'Добрий вечір! 🌙',
        'Привіт, вечірній герою! 🌆',
        'Ще працюємо? 💪',
    ],
    night: [
        'Ого, працюєш допізна! 🦉',
        'Нічна зміна? 🌃',
        'Привіт, нічний рейнджер! 🌟',
    ]
};
...
function generateTemplateMessage(ctx) {
    ...
    // Day context
    if (weekend) {
        parts.push(`Сьогодні ${dayName} — вихідний день у парку 🎢`);
    } else {
        parts.push(`Сьогодні ${dayName}.`);
    }

    // Bookings summary
    if (ctx.bookingsCount > 0) {
        const word = ctx.bookingsCount === 1 ? 'бронювання' : (ctx.bookingsCount < 5 ? 'бронювання' : 'бронювань');
        parts.push(`📊 На сьогодні ${ctx.bookingsCount} ${word} на ${formatPrice(ctx.totalRevenue)}.`);
        if (ctx.preliminaryCount > 0) {
            parts.push(`⏳ З них ${ctx.preliminaryCount} попередніх — не забудь підтвердити!`);
        }
    } else {
        parts.push('📭 Бронювань на сьогодні поки немає.');
    }

    // Tasks
    if (ctx.overdueTasks > 0) {
        parts.push(`🔴 ${ctx.overdueTasks} прострочених задач потребують уваги!`);
    } else if (ctx.pendingTasks > 0) {
        parts.push(`📋 ${ctx.pendingTasks} задач чекають виконання.`);
    } else {
        parts.push('✅ Всі задачі виконані — красунчик!');
    }

    // Streak
    if (ctx.streak > 0) {
        if (ctx.streak >= 7) {
            parts.push(`🔥 Стрік ${ctx.streak} днів поспіль! Легенда!`);
        } else if (ctx.streak >= 3) {
            parts.push(`🔥 Стрік ${ctx.streak} днів — тримай темп!`);
        }
    }

    // Animators
    if (ctx.animatorsToday > 0) {
        parts.push(`👥 Сьогодні працює ${ctx.animatorsToday} аніматорів.`);
    }
