...
check('Room timeline banquet preview uses readable labels instead of single-letter badge labels',
    ...
    && timelineBanquetRoomCardBlock.includes('timelineBanquetMarkerLabel(servingMarker)')
...
check('Room timeline banquet serving signals stay frontend-only and snapshot-backed',
    ...
    && timelineCode.includes('timelineBanquetMarkerLabel(servingMarker)')
...
check('Timeline sticky time scale masks the top scroll seam without losing mobile guards',
    ...
    && timelineConstructorCss.includes('--timeline-scale-top-shield: 6px')
    && timelineConstructorCss.includes('--timeline-scale-inline-bleed: 8px')
    && timelineConstructorCss.includes('--timeline-scale-radius: 10px')
    && timelineConstructorCss.includes('top: calc(-1 * var(--timeline-scale-top-shield, 20px))')
    && timelineConstructorCss.includes('left: calc(-1 * var(--timeline-scale-gutter, 130px))')
    && timelineConstructorCss.includes('right: calc(-1 * var(--timeline-scale-inline-bleed, 8px))')
    && timelineConstructorCss.includes('border-bottom: 1px solid var(--timeline-scale-border')
    && timelineConstructorCss.includes('border-radius: var(--timeline-scale-radius, 10px) var(--timeline-scale-radius, 10px) 0 0')
    ...
    && darkModeCss.includes('--timeline-scale-top-shield: 6px')
...
check('Booking kitchen menu uses searchable catalog controls instead of the long native select as the primary UI',
    ...
    && panelCss.includes('transform: translateY(-2px)')
...
check('Timeline create booking button opens the shared booking panel', appCode.includes("document.getElementById('newBookingBtn')?.addEventListener('click', openTimelineCreateBookingFromToolbar)") && timelineCode.includes('async function openTimelineCreateBookingFromToolbar') && timelineCode.includes('openBookingPanel(time, line.id)') && timelineCode.includes('getDefaultTimelineBookingTime'));
