...
test('booking customer selection hydrates compact lead context through canonical APIs', () => {
    ...
    const conversionStart = bookingJs.indexOf('async function applyLeadConversionContextToBookingForm');
    const conversionEnd = bookingJs.indexOf('function clearLeadConversionContextAfterBooking', conversionStart);
    ...
    const conversionBlock = bookingJs.slice(conversionStart, conversionEnd);
    assert.match(conversionBlock, /await hydrateBookingCustomerSelection\(\{ customerId: linkedCustomerId, customer: \{/);
    assert.doesNotMatch(conversionBlock, /applySelectedCustomerToBookingForm\(\{/);
