...
- Read-only QA account preflight: the available smoke account is QA-labelled, active, has Park membership and no staff profile, but its `senior_manager` role cannot issue certificates. The other account in the local secrets file is a `creator` with a staff profile and also cannot issue. An eligible isolated QA-admin exists in production, but no credential for it is present in the permitted local secrets file. No account, credential, role, or production data was changed.
