...
| Omni chat-to-lead AI draft preview | OpenAI direct | `OPENAI_API_KEY`, `OPENAI_API_BASE`, `OPENAI_OMNI_LEAD_PREVIEW_MODEL`, `OPENAI_OMNI_LEAD_PREVIEW_MAX_MESSAGES`, `OPENAI_OMNI_LEAD_PREVIEW_TIMEOUT_MS` | `/api/omni/conversations/:id/lead-assistant/preview-draft` викликає OpenAI Responses API тільки для заповнення reviewed lead draft. Flow preview-only: не створює lead/customer/task/conversation, не шле повідомлень, читає спільний latest-window slice, повертає snapshot + field→message evidence, а server validation приймає тільки inbound/customer evidence. Старий Omni lead assistant text rail лишається OpenRouter. |
...
- `/api/settings/ai/providers` повертає server-side карту provider-ів без секретів: OpenRouter shared text rail, Kie media rail, direct OpenAI assistant/menu/Omni draft-preview rail і legacy direct exceptions.
...
- CRM assistant rail, kitchen menu AI review і Omni chat-to-lead draft preview за рішенням продукту лишаються на direct OpenAI. Інші нові text/token rails мають іти через OpenRouter, якщо немає окремого product decision.
...
- `services/omniLeadAssistant.js` для Omni chat-to-lead draft preview не є legacy cleanup debt: це окремий reviewed-draft flow під direct OpenAI, із server-side evidence validation і без CRM mutation до manual save.
