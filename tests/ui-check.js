...
checkPage('staff.html', (doc, html) => {
    ...
    check('Staff schedule keeps premium HR Pulse switcher and unified panel rhythm',
        ...
        && html.includes('js/staff-schedule-shell.js?v=0.78.94')
        && html.includes('js/hr-pulse-switcher.js?v=0.78.94')
    ...
    check('Staff schedule keeps attendance out of visible shift cells',
        ...
        && !staffScheduleEmpRowBlock.includes('renderScheduleAttendanceIndicator')
        && !staffScheduleEmpRowBlock.includes('attendanceIndicator')
...
check('Profile My Day compact task cards keep bounded metadata and one unified checklist summary', profileCode.includes('let activeCabinetInlineTaskId') && profileCode.includes('function cabinetResolveActiveInlineTaskId') && profileCode.includes('function cabinetDefaultInlineTaskId') && profileCode.includes('function setCabinetActiveInlineTask') && profileCode.includes('function cabinetTaskVisibleBadges') && profileCode.includes('data-cabinet-visible-badge') && profileCode.includes('is-my-day-compact-card') && profileCode.includes('data-cabinet-active-subtask-slice') && profileCode.includes('function renderCabinetActiveSubtaskSlice') && profileCode.includes('function renderCabinetMyDaySubtaskSummary') && profileCode.includes('data-cabinet-subtask-summary') && profileCode.includes('aria-controls="cabinetSubtasksPanel') && profileCode.includes('{ showHead: !isMyDayCard }') && profilePagesCss.includes('.cabinet-task-card.is-my-day-compact-card') && profilePagesCss.includes('.cabinet-task-visible-badge') && profilePagesCss.includes('.cabinet-subtask-summary') && profilePagesCss.includes('.cabinet-subtask-active-slice') && profilePagesCss.includes('.cabinet-subtask-toggle::after'));
check('Profile My Day overdue segment renders a responsive triage shell with existing delegated actions', profileCode.includes('function renderCabinetOverdueTriageList') && profileCode.includes('function renderCabinetOverdueTriageRow') && profileCode.includes('data-cabinet-overdue-triage') && profileCode.includes('data-cabinet-overdue-triage-row') && profileCode.includes('data-cabinet-task-action="move-to-today"') && profileCode.includes('data-cabinet-task-action="reschedule-overdue"') && profileCode.includes('data-reschedule-option="custom"') && profileCode.includes('data-source-surface="profile_my_cabinet_overdue_triage"') && profileCode.includes('data-cabinet-move-target="no_date"') && profileCode.includes('data-cabinet-move-method="triage"') && profileCode.includes("button.dataset.cabinetMoveMethod || 'menu'") && profilePagesCss.includes('.cabinet-overdue-triage') && profilePagesCss.includes('container-type: inline-size') && profilePagesCss.includes('.cabinet-overdue-triage-row') && profilePagesCss.includes('grid-template-columns: minmax(220px, 1fr) minmax(0, max-content);') && profilePagesCss.includes('.cabinet-overdue-triage-actions') && profilePagesCss.includes('flex-wrap: wrap') && profilePagesCss.includes('@container (max-width: 640px)') && profilePagesCss.includes('.cabinet-overdue-triage-actions .cabinet-task-more'));
...
check('HR grouped IA keeps Pulse clean and vacancy workspace owns hiring surfaces',
    ...
    && htmlContains('hr.html', 'js/hr-pulse-switcher.js?v=0.78.94')
    ...
    && htmlContains('hr.html', 'js/staff-schedule-shell.js?v=0.78.94')
    && htmlContains('hr.html', 'js/staff-page.js?v=0.78.94')
...
check('HR Team readiness uses honest photo, Face ID, CRM, and actionable setup filters',
    ...
    && hrCode.includes("id: 'missing_onboarding_owner'")
    && hrCode.includes('function staffHasProfilePhoto')
    && hrCode.includes('function staffHasFaceDescriptor')
    && hrCode.includes('function renderTeamSetupBanner')
    && hrCode.includes('window.setTeamSetupFilter')
    && hrCode.includes('staffMatchesTeamSetupFilter(item, activeTeamSetupFilter)')
    && hrCode.includes('Фото профілю')
    && hrCode.includes('Камера / Face ID')
    && hrCode.includes('staff_face_descriptors')
    && hrCode.includes('CRM-акаунт показано як setup-пункт')
    && !hrCode.includes('Фото є')
    && !hrCode.includes('Фото не додано')
    && !hrCode.includes('Фото нема')
    && htmlContains('hr.html', 'Фото профілю')
    && htmlContains('hr.html', 'Камера / Face ID')
    && htmlContains('css/hr-page.css', '.hr-setup-banner')
