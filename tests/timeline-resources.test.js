...
test('timeline booking blocks expose width-based density display modes', () => {
    const timeline = read('js/timeline.js');
    const timelinePresentation = read('js/timeline-presentation.js');
    const css = read('css/timeline.css');

    assert.match(timeline, /function timelineBookingBlockDensity\(width\) \{[\s\S]*TimelinePresentation\.timelineBookingBlockDensity\(width\)/);
    assert.match(timelinePresentation, /function timelineBookingBlockDensity\(width\) \{[\s\S]*if \(!Number\.isFinite\(safeWidth\) \|\| safeWidth < 34\) return 'micro';[\s\S]*if \(safeWidth < 72\) return 'tiny';[\s\S]*if \(safeWidth < 132\) return 'short';[\s\S]*if \(safeWidth < 220\) return 'medium';[\s\S]*return 'wide';/);
...
test('timeline activity blocks use catalog timeline codes without truncated name fragments', () => {
    ...
    assert.match(timeline, /TimelinePresentation\.resolveTimelineActivityPresentation\(booking, renderBooking, bookingTitle, bookingTitleTail\)/);
    assert.match(timeline, /function timelineActivityBookingBlockDensity\(width, baseDensity, presentation, duration\) \{[\s\S]*TimelinePresentation\.timelineActivityBookingBlockDensity\(width, baseDensity, presentation, duration\)/);
    assert.match(timelinePresentation, /function resolveTimelineActivityPresentation\(booking = \{\}, renderBooking = null, bookingTitle = '', bookingTitleTail = '', options = \{\}\) \{/);
    assert.match(timelinePresentation, /source\.timelineCode \|\| source\.timeline_code \|\| booking\?\.timelineCode \|\| booking\?\.timeline_code/);
    assert.match(timelinePresentation, /function timelineActivityBookingBlockDensity\(width, baseDensity, presentation, duration\) \{[\s\S]*estimateTextWidth\(fullTitle\)[\s\S]*durationBadgeWidth[\s\S]*availableContentWidth[\s\S]*: 'short';/);
    ...
    assert.match(timelinePresentation, /const categoryCode = isPinata \? CATEGORY_CODES\.pinata : categoryCodeFor\(category, haystack\)/);
    assert.match(timelinePresentation, /pinataMode === 'client'[\s\S]*productCode = 'КЛ'/);
    assert.match(timelinePresentation, /pinataNumber[\s\S]*productCode = normalizePinataNumber\(pinataNumber, options\)/);
    assert.match(timelinePresentation, /categoryCode/);
    assert.match(timelinePresentation, /productCode/);
    assert.match(timelinePresentation, /compactLabel/);
    assert.match(timelinePresentation, /fullLabel/);
    assert.match(timelinePresentation, /ariaLabel/);
    ...
    assert.match(timeline, /const compactActivityLabel = activityPresentation\.compactLabel \|\| activityPresentation\.code;/);
...
test('pinata compact labels preserve operational numbers without using duration as a fallback', () => {
    const presentation = read('js/timeline-presentation.js');
    const timeline = read('js/timeline.js');
...
test('micro, short and tiny timeline activity blocks have dedicated compact CSS layout', () => {
    ...
    assert.match(css, /\.booking-block\.booking-block--micro \.timeline-micro-booking-code\s*\{[\s\S]*display:\s*flex;[\s\S]*flex-direction:\s*column;[\s\S]*flex:\s*0 0 100%;[\s\S]*width:\s*100%;[\s\S]*min-width:\s*0;[\s\S]*box-sizing:\s*border-box;[\s\S]*font-size:\s*11px;[\s\S]*text-align:\s*center;[\s\S]*text-overflow:\s*clip/);
    assert.match(css, /\.booking-block\.booking-block--micro \.timeline-micro-booking-code span\s*\{[\s\S]*display:\s*block;[\s\S]*white-space:\s*nowrap/);
    ...
    const tinyFiveCharacterTypography = css.indexOf('.booking-block.booking-block--tiny .timeline-compact-booking-label[data-max-token-length="5"] {', tinyBaseTypography);
    const tinySixCharacterTypography = css.indexOf('.booking-block.booking-block--tiny .timeline-compact-booking-label[data-max-token-length="6"] {', tinyBaseTypography);
    assert.ok(tinyFiveCharacterTypography > tinyBaseTypography, 'five-character token override follows base typography');
    assert.ok(tinySixCharacterTypography > tinyBaseTypography, 'six-character token override follows base typography');
    assert.match(css, /\.booking-block\.booking-block--short \.timeline-compact-booking-label\[data-max-token-length="4"\]\s*\{[\s\S]*font-size:\s*11px/);
    assert.match(css, /\.booking-block\.booking-block--short \.timeline-compact-booking-label\[data-max-token-length="5"\]\s*\{[\s\S]*font-size:\s*10px/);
    assert.match(css, /\.booking-block\.booking-block--short \.timeline-compact-booking-label\[data-max-token-length="6"\]\s*\{[\s\S]*font-size:\s*9px/);
    ...
    assert.match(css, /\.booking-block\.booking-block--tiny \.timeline-compact-booking-label\[data-max-token-length="4"\]\s*\{[\s\S]*font-size:\s*10px/);
    assert.match(css, /\.booking-block\.booking-block--tiny \.timeline-compact-booking-label\[data-max-token-length="5"\]\s*\{[\s\S]*font-size:\s*9\.5px/);
    assert.match(css, /\.booking-block\.booking-block--tiny \.timeline-compact-booking-label\[data-max-token-length="6"\]\s*\{[\s\S]*font-size:\s*9px/);
    assert.match(css, /\.booking-block\.booking-block--tiny:has\(\.timeline-compact-booking-label\[data-code-length="7"\]\) \.user-letter,[\s\S]*\.booking-block\.booking-block--tiny:has\(\.timeline-compact-booking-label\[data-code-length="8"\]\) \.user-letter\s*\{[\s\S]*display:\s*none/);
    ...
    assert.match(css, /body\.timeline-view-rooms \.booking-block\.is-room-timeline-activity-card\.booking-block--micro \.timeline-room-activity-title\[data-max-token-length="5"\],[\s\S]*booking-block--tiny \.timeline-room-activity-title\[data-max-token-length="5"\]\s*\{[\s\S]*font-size:\s*9\.5px/);
    assert.match(css, /body\.timeline-view-rooms \.booking-block\.is-room-timeline-activity-card\.booking-block--micro \.timeline-room-activity-title\[data-max-token-length="6"\],[\s\S]*booking-block--tiny \.timeline-room-activity-title\[data-max-token-length="6"\]\s*\{[\s\S]*font-size:\s*9px/);
    assert.doesNotMatch(css, /font-size:\s*[456]px/);
...
test('business operating profile owns shell start page and module visibility', () => {
    ...
    assert.match(auth, /await hydrateBusinessOperatingProfile\(verifiedUser, \{ sessionSnapshot: bootstrapSession \}\)/);
