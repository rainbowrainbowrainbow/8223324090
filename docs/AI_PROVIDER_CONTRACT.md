...
| Omni chat-to-lead AI draft preview | OpenAI direct | `OPENAI_API_KEY`, `OPENAI_API_BASE`, `OPENAI_OMNI_LEAD_PREVIEW_MODEL`, `OPENAI_OMNI_LEAD_PREVIEW_MAX_MESSAGES`, `OPENAI_OMNI_LEAD_PREVIEW_TIMEOUT_MS` | `/api/omni/conversations/:id/lead-assistant/preview-draft` викликає OpenAI Responses API тільки для заповнення reviewed lead draft. Flow preview-only: не створює lead/customer/task/conversation, не шле повідомлень, читає latest-window slice, повертає snapshot + field→message evidence, а server validation приймає тільки inbound/customer evidence. Старий Omni lead assistant text rail лишається OpenRouter. |
