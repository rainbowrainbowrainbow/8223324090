...
check('Profile replaces Огляд with professions hub and keeps legacy tab fallback', profileCode.includes("let activeTab = 'professions'") && profileCode.includes("renderProfilePrimaryTab('professions', 'Мої професії')") && !profileCode.includes("renderProfilePrimaryTab('profile', 'Огляд')") && profileCode.includes("requestedTab === 'profile' ? 'professions'") && profileCode.includes('function renderProfileProfessionsTab') && profileCode.includes('PROFILE_PROFESSION_GUIDES'));
check('Profile professions hub uses role, checklist and next-shift data', profileCode.includes('Професійний контекст') && profileCode.includes('Чекліст ролі') && profileCode.includes('profileProfessionChecklistTasks') && profileCode.includes('profileShiftValue(nextShift)') && !profileCode.includes('<h2>Робочий стан</h2>') && authRouteCode.includes('profile_cockpit_widgets') && authRouteCode.includes('nextShift') && authRouteCode.includes('COUNT(*)::int AS total'));
...
check('Profile my day supports overdue to today persisted move', profileCode.includes('data-cabinet-task-drag="overdue"') && profileCode.includes('data-cabinet-task-drop-target="today"') && profileCode.includes('function handleCabinetTaskDrop') && profileCode.includes('function moveCabinetTaskToToday') && profileCode.includes("profile_my_cabinet_overdue_to_today_drop") && profilePagesCss.includes('.cabinet-task-section--drop-target.is-drag-over') && profilePagesCss.includes('.cabinet-task-move-today'));
check('Profile cabinet tasker has truthful segment counts and default-collapsed composer', profileCode.includes('const CABINET_TASK_SEGMENTS') && profileCode.includes('function cabinetTaskMatchesSegment') && profileCode.includes('function cabinetSegmentCounts') && profileCode.includes('function renderCabinetTaskComposer') && profileCode.includes('data-cabinet-composer-state=') && profileCode.includes("'expanded' : 'collapsed'") && profileCode.includes('data-cabinet-composer-toggle') && profileCode.includes('data-cabinet-composer-advanced') && profileCode.includes('function setCabinetTaskComposerExpanded') && profileCode.includes('function isCabinetPersonalTask') && profileCode.includes("visibility === 'me_only'") && profileCode.includes("category === 'improvement'") && profileCode.includes('data-cabinet-active-segment') && profileCode.includes('data-cabinet-due-preset') && profilePagesCss.includes('.cabinet-task-composer.is-collapsed') && profilePagesCss.includes('[data-cabinet-composer-advanced][hidden]') && profilePagesCss.includes('.cabinet-list-head') && profilePagesCss.includes('.cabinet-task-priority'));
check('Profile my day and my tasks have deduplicated scope ownership', renderMyDayBody.includes('renderCabinetPulseCluster()') && renderMyDayBody.includes('renderCabinetTaskComposer') && renderMyTasksBody.includes('cabinet-shell--mytasks') && renderMyTasksBody.includes('cabinet-segments') && renderMyTasksBody.includes('data-cabinet-active-segment') && !renderMyTasksBody.includes('renderCabinetPulseCluster()') && !renderMyTasksBody.includes('renderCabinetTaskComposer') && !profileCode.includes("setCabinetQuickMode('tasks')") && profilePagesCss.includes('.cabinet-toolbar-actions') && profilePagesCss.includes('.cabinet-link-btn--secondary'));
...
check('Sidebar no longer exposes standalone Programs entrypoints', !sidebarCode.includes("href: '/programs'") && !sidebarCode.includes("href: '/programs#catalogs'"));
...
check('Task quick widgets split completed-today and active today/undated counts', profileCode.includes('function cabinetTaskQuickCounts') && profileCode.includes('виконано сьогодні') && profileCode.includes('cabinet-quick-split') && profileCode.includes('cabinet-quick-half--completed') && profileCode.includes('cabinet-quick-half--remaining') && profilePagesCss.includes('.cabinet-quick-divider') && sidebarCode.includes('/api/tasks/my-cabinet') && sidebarCode.includes('_isSidebarTaskCompletedToday') && sidebarCode.includes('виконано сьогодні') && sidebarCode.includes('focusChipTasksDoneValue') && sidebarCode.includes('focus-chip-task-split') && sidebarAuroraCss.includes('.focus-chip-task-divider') && htmlContains('routes/tasks.js', 'completed: quickStats.done_today') && !sidebarCode.includes("Number(tasks.assigned || 0) + Number(tasks.in_progress || 0)"));
...
check('Role quick access orders animator Afisha before timeline and gives managers certificate issuance', authCode.includes("quickAccess: ['/afisha', '/', '/tasks', '/chat', '/training']") && authCode.includes("art_director: {\n        startPage: '/art'") && authCode.includes("manager: {\n        startPage: '/dashboard'") && authCode.includes("quickAccess: ['/dashboard', '/certificates/new', '/', '/sales-funnel', '/tasks', '/chat']") && authCode.includes("senior_manager: {\n        startPage: '/dashboard'") && authCode.includes("vice_director: {\n        startPage: '/dashboard'") && authCode.includes("director: {\n        startPage: '/dashboard'") && sidebarCode.includes("timeline:       [..._ADMIN_UP, 'reception', 'animator'") && featureRegistryCode.includes("id: 'afisha.events'") && featureRegistryCode.includes("href: '/afisha'") && !featureRegistryCode.includes("id: 'products.programs'"));
