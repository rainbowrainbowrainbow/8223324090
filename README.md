...
Task AI rollout evidence is collected through sanitized metadata only. Prefer `npm run task-ai-rollout-collect -- --service <service> --deployment-id <id> --version <version> --sha <40-char-sha> --stage <stage> --scope <single|bundle> --database`: the operator wrapper reads Railway deploy/HTTP logs in memory, never saves raw logs, and writes redacted JSON/Markdown artifacts under `output/task-ai-rollout/` or `output/task-ai-bundle-rollout/`. A dedicated process-local read-only `TASK_AI_ROLLOUT_DATABASE_URL` is required for DB safety gates; `DATABASE_URL` is intentionally ignored. Reports distinguish true zero traffic (`HOLD_INSUFFICIENT_TRAFFIC`) from missing structured telemetry despite HTTP traffic (`TELEMETRY_GAP`), and pass only with 30 actionable non-fallback proposals or a 24-hour exact-deployment window containing real preview attempts plus clean DB/provider gates. Paid real Luna eval remains separate: `npm run eval:task-ai:live` requires explicit `TASK_AI_LIVE_EVAL_CONFIRM=RUN_LUNA_PROPOSAL_EVAL` and never runs in CI/test.
