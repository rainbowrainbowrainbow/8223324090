...
async function runAudit() {
  ...
  for (const page of PAGES_TO_AUDIT) {
    ...
    const scores = {
      page: page.name,
      url: page.url,
      performance: Math.round(result.ljs.categories.performance.score * 100),
      accessibility: Math.round(result.ljs.categories.accessibility.score * 100),
      bestPractices: Math.round(result.ljs.categories['best-practices'].score * 100),
      seo: Math.round(result.ljs.categories.seo.score * 100),
      lcp: result.ljs.audits['largest-contentful-paint'].numericValue,
      cls: result.ljs.audits['cumulative-layout-shift'].numericValue,
      fcp: result.ljs.audits['first-contentful-paint'].numericValue,
      ttfb: result.ljs.audits['server-response-time'].numericValue,
    };
