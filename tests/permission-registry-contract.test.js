...
test('registry describes the current page catalog and active action contract', () => {
    ...
    assert.equal(registry.ACTION_PERMISSIONS.length, 49);
    assert.equal(actionKeys.length, 45);
...
test('sidebar links are fully represented by page permission entries', () => {
    ...
    assert.deepEqual(registry.PAGE_PERMISSION_BY_KEY['/designer'].sidebarLinks, []);
