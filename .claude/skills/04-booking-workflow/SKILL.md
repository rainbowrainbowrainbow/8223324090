...
| DRAFT | HOLD | submit() | eventPublished, capacityAvailable | Reserve capacity, set holdExpiresAt (+30min), notify client |
| HOLD | PENDING_PAYMENT | initiatePayment() | holdNotExpired | Create payment intent |
| HOLD | CANCELLED | holdExpired() | holdExpiresAt < now() | Release capacity, notify client |
| HOLD | CANCELLED | cancel() | - | Release capacity |
| PENDING_PAYMENT | CONFIRMED | depositPaid() | paymentVerified, amount >= depositAmount | Update paidAt, notify client + manager |
| PENDING_PAYMENT | CANCELLED | cancel() | - | Void payment, release capacity |
| PENDING_PAYMENT | CANCELLED | paymentFailed() | - | Log error, release capacity, notify client |
| CONFIRMED | PAID | fullPayment() | totalPaid >= totalPrice | Update paidAt, notify client |
| CONFIRMED | CANCELLED | cancel() | - | Calculate refund, release capacity, notify |
| PAID | COMPLETED | eventCompleted() | event.dateEnd < now() | Update stats, request feedback |
| PAID | CANCELLED | cancel() | - | Calculate refund, release capacity, notify |
| PAID | NO_SHOW | markNoShow() | managerOnly | No refund, update stats |
| CANCELLED | REFUNDED | processRefund() | hasEligiblePayments | Initiate refund via payment provider |
...
const transitions: Record<string, BookingTransition> = {
  ...
  cancel: {
    from: ['HOLD', 'PENDING_PAYMENT', 'CONFIRMED', 'PAID'],
    to: 'CANCELLED',
    guards: [],
    effects: [
      'releaseCapacity',
