...
const WEEKLY_PLAN_PLATFORMS = Object.freeze(['instagram', 'telegram']);
const WEEKLY_PLAN_TOPICS = Object.freeze(['animation', 'quest', 'birthday', 'show', 'masterclass']);
let isMarketingScheduledPublishRunning = false;
let isMarketingWeeklyPlanRunning = false;
let lastMarketingWeeklyPlanDate = null;
...
async function publishPost(postId) {
    ...
    if (!['approved', 'scheduled'].includes(post.status)) {
        throw new Error(`Не можна опублікувати пост зі статусом "${post.status}". Потрібен статус "approved" або "scheduled".`);
    }
    ...
    if (hasErrors) {
        ...
        try {
            ...
            if (ch.rows[0]) {
                const msg = await sendBotMessage(ch.rows[0].id, `❌ Помилка постинга: "${post.title}" — ${errMsg}`);
                broadcastToChannel(ch.rows[0].id, 'chat:message', { channelId: ch.rows[0].id, message: msg });
            }
        } catch (chatErr) { log.warn('Publish alert failed:', chatErr.message); }
    } else {
...
async function runMarketingScheduledPublish(deps = {}) {
    if (isMarketingScheduledPublishRunning) {
        return { skipped: true, reason: 'overlap', results: [], count: 0 };
    }

    isMarketingScheduledPublishRunning = true;
    try {
        const publish = deps.publishScheduled || publishScheduled;
        const results = await publish();
        return { skipped: false, results, count: Array.isArray(results) ? results.length : 0 };
    } finally {
        isMarketingScheduledPublishRunning = false;
    }
}
...
function calculateNextWeeklyPlanTarget(now) {
    const nextWeekDate = new Date(now);
    nextWeekDate.setUTCDate(nextWeekDate.getUTCDate() + 7);
    const year = nextWeekDate.getUTCFullYear();
    const jan1 = new Date(Date.UTC(year, 0, 1));
    const targetDate = new Date(Date.UTC(year, nextWeekDate.getUTCMonth(), nextWeekDate.getUTCDate()));
    const weekNumber = Math.ceil((((targetDate - jan1) / 86400000) + jan1.getUTCDay() + 1) / 7);
    return { weekNumber, year };
}

async function runMarketingWeeklyPlanScheduler(deps = {}) {
    const now = deps.now || new Date();
    const todayStr = dateKeyUtc(now);
    ...
    if (lastMarketingWeeklyPlanDate === todayStr) {
        return { skipped: true, reason: 'already_ran_today', date: todayStr };
    }
    if (isMarketingWeeklyPlanRunning) {
        return { skipped: true, reason: 'overlap' };
    }

    isMarketingWeeklyPlanRunning = true;
    try {
        const generate = deps.generateWeeklyPlan || generateWeeklyPlan;
        const target = calculateNextWeeklyPlanTarget(now);
        const posts = await generate(
            target.weekNumber,
            target.year,
            [...WEEKLY_PLAN_PLATFORMS],
            [...WEEKLY_PLAN_TOPICS],
            null
        );
        lastMarketingWeeklyPlanDate = todayStr;
        return {
            skipped: false,
            posts,
        ...
        isMarketingWeeklyPlanRunning = false;
    }
}
...
module.exports = {
    ...
    publishPost, publishScheduled, runMarketingScheduledPublish,
    runMarketingWeeklyPlanScheduler, getStatus,
    AGENT_CONFIG
};
