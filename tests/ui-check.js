...
checkPage('index.html', (doc, html) => {
    ...
    const timelineCompactCommandCss = sourceBlock(
        ...
        '/* v0.77.82: filters stay hidden behind one compact popover trigger. */'
    );
    ...
    check('Timeline action menu is contextual and does not duplicate sidebar navigation',
        ...
        && doc.querySelector('.header .timeline-header-actions > #historyBtn.btn-history.timeline-header-history-btn')
    ...
    check('Timeline history opens as a primary topbar action with shared history styling',
        doc.querySelector('.header .timeline-header-actions > #historyBtn.btn-history.timeline-header-history-btn')
        && !doc.querySelector('.schedule-command-center #historyBtn')
    ...
    check('Timeline header keeps view panel, ordered topbar actions, and a utility-row view trigger',
        ...
        && !!doc.querySelector('#timelineViewPanelToggle #timelineViewPanelSummary.timeline-filter-summary')
        ...
        && Array.from(doc.querySelectorAll('.timeline-view-panel-label')).map(el => el.textContent.trim()).join('|') === 'Статус|Період|Тип|Масштаб'
        ...
        && htmlContains('css/responsive.css', 'v0.77.82: filters stay hidden behind one compact popover trigger.')
        && htmlContains('css/responsive.css', '.timeline-view-panel')
        && htmlContains('css/responsive.css', '.timeline-view-panel[hidden]')
        && htmlContains('css/responsive.css', 'max-inline-size: min(620px, calc(100vw - 48px))')
    ...
    check('Timeline header regression guard forbids old search, compact, label, and second-row contracts',
        ...
        && timelineViewPanelRule.includes('max-inline-size: min(620px, calc(100vw - 48px))')
        && timelineInlineViewPanelRule.includes('position: absolute !important')
        && timelineInlineViewPanelRule.includes('top: calc(100% + 8px) !important')
        && timelineInlineViewPanelRule.includes('width: min(620px, calc(100vw - 48px)) !important')
        && timelineInlineViewPanelRule.includes('max-width: min(620px, calc(100vw - 48px)) !important')
    ...
    check('Timeline view panel filters keep exact visual order and direct handler hooks',
        ...
        }).join('|') === 'status|period|type|zoom'
        && !doc.querySelector('.timeline-header-filters-label')
        && !doc.querySelector('.timeline-header-filter-icon--sliders')
        && !doc.querySelector('.timeline-header-filters .timeline-compact-toggle')
        && !doc.getElementById('compactModeToggle')
        && Array.from(doc.querySelectorAll('.timeline-header-filter-group > .timeline-view-panel-label')).map(el => el.textContent.trim()).join('|') === 'Статус|Період|Тип|Масштаб'
        ...
        && !doc.querySelector('.timeline-header-filters #logoutBtn'));
    check('Timeline view panel keeps status, period, type, and scale controls distributed',
    ...
    check('Timeline view panel filter toolbar is theme-aware without losing light active tokens',
        ...
        && timelineViewPanelRule.includes('box-shadow: 0 18px 42px')
        && htmlContains('css/responsive.css', '--timeline-header-filter-control-bg: var(--eg-panel-bg-soft')
        && htmlContains('css/responsive.css', '--timeline-header-filter-border: var(--eg-border-default')
    ...
    check('Timeline view panel opens as a compact filter popover from the date command row',
        ...
        && timelineViewPanelRule.includes('position: absolute')
        && timelineViewPanelRule.includes('width: min(620px, calc(100vw - 48px))')
        && timelineViewPanelRule.includes('max-width: min(620px, calc(100vw - 48px))')
        && timelineInlineViewPanelRule.includes('position: absolute !important')
        && timelineInlineViewPanelRule.includes('inset: auto auto auto 0 !important')
        && timelineInlineViewPanelRule.includes('top: calc(100% + 8px) !important')
        && timelineInlineViewPanelRule.includes('width: min(620px, calc(100vw - 48px)) !important')
        && timelineInlineViewPanelRule.includes('max-inline-size: min(620px, calc(100vw - 48px)) !important')
        && timelineViewPanelHiddenRule.includes('display: none !important')
        ...
        && htmlContains('css/responsive.css', 'grid-template-columns: minmax(240px, 1.25fr) minmax(150px, 0.75fr)')
    ...
    check('Timeline filter popover and topbar actions have an overflow-safe contract',
        ...
        && timelineHeaderFiltersRule.includes('grid-template-columns: minmax(240px, 1.25fr) minmax(150px, 0.75fr)')
    ...
    check('Timeline browser smoke guards header visibility and 15-minute geometry thresholds',
        ...
        && timelineBrowserSmokeCode.includes('#timelineViewPanel [data-timeline-view="rooms"]')
        ...
        && timelineBrowserSmokeCode.includes('opening filters does not grow the command center')
        && timelineBrowserSmokeCode.includes('opening filters does not push timeline down')
        && timelineBrowserSmokeCode.includes('openMetrics.viewPanelWidth <= Math.min(620, openMetrics.viewportWidth - 48) + 2')
    ...
    check('Timeline toolbar exposes accessible pressed, loading, and focus states',
        ...
        && doc.querySelector('#historyBtn')?.closest('.timeline-header-actions')
        && !doc.getElementById('digestBtn')
    ...
    check('Timeline assistant rail late CSS keeps ordered topbar actions styled',
        ...
        && timelineInlineViewPanelRule.includes('z-index: 140 !important'));
