...
- Замінено фонове зображення сертифіката на нову версію з оновленим розташуванням логотипу
- QR-код перенесено з центру (перекривав персонажа) у нижній лівий кут
- Інфо-блок (дата, умови, телефон) розміщено поруч з QR-кодом
- Зменшено QR з 216px до 150px для кращого вписування
- Cache busting: всі CSS/JS версії оновлено до 8.6.1

---

## 2026-02-14 — v8.6.0 Розумний розподіл

**Іменинники — redesign:**
- Birthday блоки: pill-форма (border-radius: 18px, height: 36px) замість звичайних прямокутників
- Яскравий градієнт (amber → orange → red) з білою рамкою 2px
- Emoji 🎂 всередині блоку для миттєвого впізнавання
- Пульсуюча анімація кільця (`@keyframes birthday-pulse`) замість subtle glow
- Жирний title (font-weight: 800) + час у напівпрозорому бейджі з заокругленням
- Dark mode: зменшена прозорість рамки для кращої видимості

**Авто-розподіл афіші перед дайджестами:**
- Виділено `distributeAfishaForDate(date)` з HTTP handler в окрему reusable функцію
- Викликається автоматично перед `buildAndSendDigest()` (щоденний дайджест)
- Викликається автоматично перед `sendTomorrowReminder()` (вечірнє нагадування)
- Lazy require через `getDistributeAfisha()` для уникнення circular dependency (routes/afisha ↔ services/scheduler)
- Non-blocking: якщо distribute не вдається (no_animators/no_events), дайджест все одно відправляється

**Техніка:**
- Cache busting: всі CSS/JS версії оновлено до 8.6.0
- Export `router.distributeAfishaForDate` для внутрішнього виклику з scheduler
- Birthday blocks: `.birthday-icon` span + умовний innerHTML в `createAfishaBlock()`
