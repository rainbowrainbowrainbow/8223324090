...
checkPage('copilot.html', (doc) => {
    ...
    check('Copilot new case uses CRM prompt modal instead of direct native prompt', copilotPageCode.includes("await promptModal('Назва кейсу:'") && !copilotPageCode.includes("const title = prompt('Назва кейсу:')"));
});
...
check('Settings Maysternya line creation uses CRM prompt modal before native fallback', settingsCode.includes("await promptModal('Назва спеціаліста або кабінету'") && settingsCode.indexOf("await promptModal('Назва спеціаліста або кабінету'") < settingsCode.indexOf("window.prompt('Назва спеціаліста або кабінету'"));
...
check('CRM assistant confirmations use shared CRM modal helper before native fallback', assistantFoundationCode.includes('function confirmAssistantAction') && assistantFoundationCode.includes('await confirmAssistantAction(action.label') && assistantFoundationCode.includes("await confirmAssistantAction(`${route.label || 'Підтвердити дію'}?${title}`") && !assistantFoundationCode.includes('const ok = window.confirm(action.label)'));
