...
const DashboardPage = (() => {
    ...
    const BOARD_CONTENT_PRESETS = {
        ...
        story: {
            ...
            notes: [
                ...
                ['Дотиск', 'Що відбувається після контакту']
            ]
        }
    };
    ...
    function requestWidgetData(type, requestContext = dashboardWidgetRequestContext(type)) {
        ...
        const headers = { 'Authorization': 'Bearer ' + localStorage.getItem('pzp_token') };
        const transport = type === 'funnel'
            ? fetch(dashboardScopedApiUrl('/api/dashboard/widgets/funnel'), { headers })
            : fetch(requestContext.url, { headers });
