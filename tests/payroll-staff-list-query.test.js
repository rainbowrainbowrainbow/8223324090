...
test('payroll staff discovery uses the current active core roster and fails closed', () => {
    ...
    assert.match(fetchStaffListSource, /Fail closed/);
...
test('legacy staff payroll endpoint also uses the current active core roster', () => {
    ...
    assert.match(staffPayrollRouteSource, /activeScheduleStaffWhere\('s', '\$1::date'\)/);
