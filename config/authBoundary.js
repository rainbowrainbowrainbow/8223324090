const INTEGRATION_AUTH_CONTRACTS = Object.freeze({
    ...
    hermesApi: { owner: 'hermes', authentication: 'API key or bearer secret', guardFiles: [{ file: 'middleware/hermesAuth.js', needles: ['function createHermesAuthMiddleware', 'timingSafeSecretEqual', 'function sanitizeHermesMachineActor'] }, { file: 'routes/hermes.js', needles: ['router.use(authMiddleware)', 'router.use(normalizeHermesMachineActor)'] }, { file: 'routes/hermes-schedule.js', needles: ['assertHermesScheduleAccess', "req.integration?.id !== HERMES_INTEGRATION_ID"] }], testFiles: ['tests/hermes-auth.test.js', 'tests/hermes-schedule-routes.test.js', 'tests/auth-boundary.test.js'] },
...
const PUBLIC_API_ROUTES = [
    ...
    {
        method: 'POST',
        regex: /^\/omni\/bridge\/v1\/(?:heartbeat|events|commands\/pull|commands\/[0-9a-f-]{36}\/result)$/,
        label: 'POST /omni/bridge/v1/*',
        examplePath: '/omni/bridge/v1/heartbeat',
        owner: 'omnichannel',
        integrationContract: 'viberPersonalBridge',
        reason: 'Viber Personal Bridge uses an exact connector-scoped Bearer credential and immutable bridge/account/business scope instead of a CRM user JWT.'
    },
    {
        method: 'POST',
