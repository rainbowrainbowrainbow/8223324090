...
check('Dashboard workspace exposes Ukrainian tool labels, snap presets and compact top options', dashboardPageCode.includes('function setBoardSnapMode') && dashboardPageCode.includes('normalizeBoardSnapMode') && dashboardPageCode.includes('BOARD_SNAP_LABELS') && dashboardPageCode.includes('AI-помічник дошки') && dashboardHtml.includes('Віджети, дошка й малювання разом') && !dashboardHtml.includes('Workspace</button>') && !dashboardHtml.includes('Reset view') && dashboardCss.includes('--workspace-selection-ring') && dashboardCss.includes('.board-tool-snap-presets') && dashboardCss.includes('.dashboard-workspace-toolbar .board-tool-options') && dashboardPageCode.includes('data-workspace-module="true"'));
...
check('HR exposes account center with account creation, profile, staff binding, and password controls', htmlContains('hr.html', 'data-tab="accounts"') && htmlContains('hr.html', 'accountCenterList') && htmlContains('hr.html', 'accountCreateBtn') && hrCode.includes('function loadAccountCenter') && hrCode.includes('function canManageAccountSecurity') && hrCode.includes('openAccountCreateModal') && hrCode.includes('function openAccountProfileModal') && hrCode.includes('function loadAccountStaffOptions') && hrCode.includes('/api/users/${encodeURIComponent(userId)}/profile') && hrCode.includes('function openAccountPasswordModal') && hrCode.includes('/api/users/${encodeURIComponent(userId)}/reset-password') && hrCode.includes('function deactivateKarinaAccounts') && htmlContains('routes/users.js', 'ACCOUNT_MANAGER_ROLES') && htmlContains('routes/users.js', "router.get('/staff-options'") && htmlContains('routes/users.js', "router.patch('/:id/profile'"));
