...
checkPage('customers.html', (doc, html) => {
    ...
    check('Customer page permits browser zoom for accessibility', html.includes('<meta name="viewport" content="width=device-width, initial-scale=1.0">') && !html.includes('maximum-scale=1.0') && !html.includes('user-scalable=no'));
    check('Customer toolbar promotes create while grouping protected import/export actions', html.indexOf('id="addCustomerBtn"') > -1 && html.indexOf('id="addCustomerBtn"') < html.indexOf('id="customerActionsMenu"') && !!doc.getElementById('customerActionsMenuBtn') && !!doc.getElementById('customerActionsMenuList') && customerPageCode.includes('function syncCustomerActionsMenu') && customerPageCode.includes('customerActionMenuItems().some(isCustomerActionVisible)') && customerPageCss.includes('.customer-actions-menu-list') && customerPageCss.includes('.customer-actions-menu-item:focus-visible'));
    check('Customer list uses visible search, labelled filters, mobile filter count, and removable chips', !!doc.querySelector('label[for="customerSearchInput"]') && !!doc.getElementById('customerFilterToggle') && !!doc.getElementById('customerFilterCount') && !!doc.getElementById('customerFilterChips') && !!doc.getElementById('customerFiltersPanel') && ['tagFilter', 'sourceFilter', 'sortFilter', 'dateFromFilter', 'dateToFilter'].every(id => !!doc.querySelector(`label[for="${id}"]`)) && customerPageCode.includes('function getCustomerActiveFilterChips') && customerPageCode.includes("f.sortBy && f.sortBy !== 'updated_at'") && customerPageCode.includes('data-customer-filter-remove') && customerPageCode.includes('data-customer-filter-reset') && customerPageCss.includes('.customer-filter-count') && customerPageCss.includes('.customer-filters-panel'));
    check('Customer list keeps one canonical table render while adding mobile compact rows', !!doc.querySelector('.customer-list-table-wrap .customer-list-table') && customerPageCode.includes('class="customer-list-row" data-id="${c.id}" tabindex="0"') && customerPageCode.includes('showCustomerDetail(id);') && customerPageCode.includes("event.key !== 'Enter' && event.key !== ' '") && customerPageCode.includes('customer-list-cell--phone') && customerPageCode.includes('customer-list-cell--last-visit') && customerPageCode.includes('customer-list-cell--visits') && !customerPageCode.includes('mobileCustomers') && !customerPageCode.includes('fetchMobileCustomers'));
    check('Customer list desktop table has sticky header and numeric alignment guards', customerPageCss.includes('.customer-list-table-wrap') && customerPageCss.includes('max-height: min(72vh, 760px)') && customerPageCss.includes('.customer-list-table thead th') && customerPageCss.includes('position: sticky') && customerPageCss.includes('.customer-list-col--number') && customerPageCss.includes('.customer-list-cell--numeric') && doc.querySelectorAll('.customer-list-col--number').length >= 2);
    check('Customer list mobile layout removes horizontal table scanning without hiding open-card action', customerPageCss.includes('@media (max-width: 768px)') && customerPageCss.includes('grid-template-areas:') && customerPageCss.includes('"identity phone"') && customerPageCss.includes('"last visits"') && customerPageCss.includes('.customer-list-cell--secondary') && customerPageCss.includes('display: none') && customerPageCss.includes('.customer-list-table .customer-name') && customerPageCss.includes('overflow-wrap: anywhere'));
...
checkPage('certificates.html', (doc, html) => {
    ...
    check('Certificates page exposes mobile read-only check surface', !!doc.getElementById('certificatesCheckView') && !!doc.getElementById('certificateCheckForm') && !!doc.getElementById('certificateCheckCode') && !!doc.getElementById('certificateCheckResult') && !html.includes('data-cert-redeem'));
    ...
    check('Certificates routes are explicit before SPA fallback', htmlContains('server.js', "app.get(['/certificates', '/certificates/new', '/certificates/batch', '/certificates/check']") && htmlContains('server.js', "res.sendFile(path.join(__dirname, 'certificates.html'));"));
    ...
    check('Certificates check route retains QR code through login and classifies Kyiv-date validity', certificatePageCode.includes("path.endsWith('/check')") && certificatePageCode.includes('CERTIFICATE_CHECK_CODE_KEY') && certificatePageCode.includes('rememberAuthReturnRoute') && certificatePageCode.includes("timeZone: 'Europe/Kyiv'") && certificatePageCode.includes('CHECK_STATE_META'));
...
checkPage('staff.html', (doc, html) => {
    ...
    check('Staff schedule keeps premium HR Pulse switcher and unified panel rhythm',
        ...
        && html.includes('js/staff-schedule-shell.js?v=0.82.19')
        && html.includes('js/hr-pulse-switcher.js?v=0.82.19')
...
checkPage('tasks.html', (doc, html) => {
    ...
    check('Tasks create success toast is compact and carries open/edit paths', html.includes('js/sound-engine.js') && tasksCode.includes('function showTaskCreateSuccessToast') && tasksCode.includes('window.TaskCreate.buildCreateNotification') && tasksCode.includes("label: 'Відкрити'") && tasksCode.includes("label: 'Редагувати'") && tasksCode.includes('payload.type || \'success\'') && tasksCode.includes('postCreateWarningCount') && taskCreateCodeForTasks.includes('function buildCreateNotification') && taskCreateCodeForTasks.includes('Чекліст:') && taskCreateCodeForTasks.includes("type: warningCount > 0 ? 'warning' : 'success'") && !tasksCode.includes('Задачу створено, але частину деталей треба перевірити'));
...
const myDayHabitsCodeForProfileChecks = fs.readFileSync(path.join(ROOT, 'js', 'my-day-habits.js'), 'utf8');
...
check('Profile task composer exposes an obvious editable field and persistent inline validation', profileCode.includes('Наприклад: підготувати кошторис до п’ятниці') && profileCode.includes('cabinetTaskTitleGuidance cabinetTaskComposerStatus') && profileCode.includes('function setCabinetTaskTitleGuidance') && profileCode.includes("setCabinetTaskTitleGuidance(titleError, 'error')") && profileCode.includes("setCabinetTaskTitleGuidance('')") && profileCode.includes('>Додати задачу</button>') && profilePagesCss.includes('.cabinet-capture .cabinet-task-title-field textarea:focus-visible') && profilePagesCss.includes('box-sizing: border-box') && profilePagesCss.includes('.cabinet-task-title-guidance.error') && profilePagesCss.includes('.cabinet-task-ai-fill.task-ai-draft-trigger'));
check('Tasks and Profile blocked completion reveal and highlight unfinished subtasks', tasksPageCodeForProfileChecks.includes('data-task-completion-blocked="subtasks"') && tasksPageCodeForProfileChecks.includes('function revealBlockedTaskSubtasks') && tasksPageCodeForProfileChecks.includes('data-task-subtask-blocked-attention="true"') && profileCode.includes('data-cabinet-completion-blocked="subtasks"') && profileCode.includes('function revealBlockedCabinetSubtasks') && profileCode.includes('data-cabinet-subtask-blocked-attention="true"') && pagesTasksCss.includes('.task-card-subtask-item.is-blocked-attention') && profilePagesCss.includes('.cabinet-subtask-inline-item.is-blocked-attention'));
check('My Day minute habits use a bounded responsive stepper with local save states', myDayHabitsCodeForProfileChecks.includes('function parseHabitNumericValue') && myDayHabitsCodeForProfileChecks.includes("numeric > 1440") && myDayHabitsCodeForProfileChecks.includes('data-my-day-habit-step="-1"') && myDayHabitsCodeForProfileChecks.includes('data-my-day-habit-step="1"') && myDayHabitsCodeForProfileChecks.includes('data-my-day-habit-save=') && myDayHabitsCodeForProfileChecks.includes('setHabitEntryBusy(entry, true)') && myDayHabitsCodeForProfileChecks.includes("setHabitEntryFeedback(entry, 'pending'") && myDayHabitsCodeForProfileChecks.includes("setHabitEntryFeedback(entry, 'error'") && myDayHabitsCodeForProfileChecks.includes("setHabitEntryFeedback(entry, 'success'") && myDayHabitsCodeForProfileChecks.includes("event.key !== 'Enter'") && profilePagesCss.includes('.my-day-habit-stepper') && profilePagesCss.includes('.my-day-habit-entry-status.is-error') && profilePagesCss.includes('.my-day-habit-entry-status.is-success') && profilePagesCss.includes('@media (max-width: 640px)'));
...
const criticalJS = [
    ...
    'js/timeline-cache.js', 'js/timeline-presentation.js', 'js/timeline-resource-identity.js', 'js/timeline-banquet-inspector-helpers.js', 'js/timeline.js', 'js/settings.js',
...
check('Auth stores refresh sessions, refreshes verify, and revokes logout sessions',
    ...
    && apiCodeForAuthSession.includes('function apiRefreshAuthSession')
    && apiCodeForAuthSession.includes('`${API_BASE}/auth/refresh`')
    && apiCodeForAuthSession.includes('const refreshResult = await apiRefreshAuthSession()')
    && apiCodeForAuthSession.includes('const refreshedToken = refreshResult.accessToken')
...
check('Room medium and wide activity titles consume available card width', timelineConstructorCss.includes('booking-block--medium .timeline-room-activity-title') && timelineConstructorCss.includes('booking-block--wide .timeline-room-activity-title') && timelineConstructorCss.includes('flex: 1 1 auto') && timelineConstructorCss.includes('width: 0'));
...
check('Timeline booking blocks expose stable width-based density display modes', timelineCode.includes('function timelineBookingBlockDensity(width)') && timelinePresentationCode.includes("safeWidth < 34) return 'micro'") && timelinePresentationCode.includes("safeWidth < 72) return 'tiny'") && timelinePresentationCode.includes("safeWidth < 132) return 'short'") && timelinePresentationCode.includes("safeWidth < 220) return 'medium'") && timelinePresentationCode.includes("return 'wide'") && timelineCode.includes('let bookingBlockDensity = timelineBookingBlockDensity(width)') && timelineCode.includes('booking-block--${bookingBlockDensity}') && timelineConstructorCss.includes('.booking-block--micro') && timelineConstructorCss.includes('.booking-block--tiny') && timelineConstructorCss.includes('.booking-block--short') && timelineConstructorCss.includes('.booking-block--medium') && timelineConstructorCss.includes('.booking-block--wide'));
check('Timeline activity blocks use shared product presentation contract with full hover text', timelineCode.includes('function timelineActivityPresentation(booking, renderBooking') && timelineCode.includes('TimelinePresentation.resolveTimelineActivityPresentation') && timelinePresentationCode.includes('function resolveTimelineActivityPresentation') && timelinePresentationCode.includes('categoryCode') && timelinePresentationCode.includes('productCode') && timelinePresentationCode.includes('compactLabel') && timelinePresentationCode.includes('fullLabel') && timelinePresentationCode.includes('tooltip') && timelinePresentationCode.includes('ariaLabel') && timelineCode.includes('function timelineFallbackActivityCode(booking, renderBooking') && timelineCode.includes('function timelineActivityBookingBlockDensity(width, baseDensity, presentation, duration)') && timelineCode.includes('timelineActivityBookingBlockDensity(width, bookingBlockDensity, activityPresentation, effectiveDuration)') && timelinePresentationCode.includes("productCode = 'КЛ'") && timelineCode.includes('const compactActivityLabel = activityPresentation.compactLabel || activityPresentation.code') && timelineCode.includes('const roomActivityMainLabel = !isCompactActivityBlock && bookingTitleTail') && timelineCode.includes("const roomActivityCanShowDetail = bookingBlockDensity !== 'micro' && bookingBlockDensity !== 'tiny'") && timelineCode.includes("const roomActivityDetailLabel = roomActivityCodeRepeatsTitle && isCompactActivityBlock ? '' : bookingTitleTail") && timelineCode.includes('const miniAccessibilityLabel = miniTitleParts.join') && timelineCode.includes('aria-label="${escapeHtml(miniAccessibilityLabel)}"') && timelineCode.includes('const normalizedBookingTitleText =') && timelineCode.includes("block.setAttribute('title', fullBookingLabel)") && timelineCode.includes('data-timeline-category-code') && timelineCode.includes('class="timeline-micro-booking-code"') && timelineCode.includes('class="timeline-compact-booking-label"') && !timelineCode.includes('timeline-compact-booking-tail'));
check('Micro, short and tiny timeline activity blocks keep readable floors without artificial widths',
    timelineConstructorCss.includes('.booking-block.booking-block--micro,')
    && timelineConstructorCss.includes('.booking-block.booking-block--short,')
    && timelineConstructorCss.includes('.booking-block.booking-block--tiny')
    && timelineConstructorCss.includes('body.timeline-dashboard-page .booking-block.booking-block--tiny')
    && timelineConstructorCss.includes('padding: 5px 3px')
    && timelineConstructorCss.includes('body.timeline-dashboard-page .booking-block.booking-block--micro')
    && timelineConstructorCss.includes('padding: 0 !important')
    && timelineConstructorCss.includes('.booking-block.booking-block--micro .timeline-micro-booking-code')
    && timelineConstructorCss.includes('flex-direction: column')
    && timelineConstructorCss.includes('.booking-block.booking-block--short .timeline-compact-booking-label[data-max-token-length="4"]')
    && timelineConstructorCss.includes('.booking-block.booking-block--short .timeline-compact-booking-label[data-max-token-length="5"]')
    && timelineConstructorCss.includes('.booking-block.booking-block--short .timeline-compact-booking-label[data-max-token-length="6"]')
    && timelineConstructorCss.includes('.booking-block.booking-block--short:has(.timeline-compact-booking-label[data-code-length="7"]) .timeline-compact-booking-main')
    && timelineConstructorCss.includes('.booking-block.booking-block--short:has(.timeline-compact-booking-label[data-code-length="8"]) .user-letter')
    && timelineConstructorCss.includes('max-width: 100%')
    && timelineConstructorCss.includes('.booking-block.booking-block--tiny .timeline-compact-booking-label[data-max-token-length="4"]')
    && timelineConstructorCss.includes('.booking-block.booking-block--tiny .timeline-compact-booking-label[data-max-token-length="5"]')
    && timelineConstructorCss.includes('.booking-block.booking-block--tiny .timeline-compact-booking-label[data-max-token-length="6"]')
    && timelineConstructorCss.includes('.booking-block.booking-block--tiny:has(.timeline-compact-booking-label[data-code-length="7"]) .user-letter')
    && timelineConstructorCss.includes('.booking-block.booking-block--tiny:has(.timeline-compact-booking-label[data-code-length="8"]) .user-letter')
    && timelineConstructorCss.includes('font-size: 9.5px')
    && timelineConstructorCss.includes('font-size: 10px')
    && timelineConstructorCss.includes('font-size: 9px')
    && timelineCode.includes('data-max-token-length="${escapeHtml(String(microLabelMetrics.maxTokenLength))}"')
    && timelineCode.includes('data-max-token-length="${escapeHtml(String(compactLabelMetrics.maxTokenLength))}"')
    && !timelineConstructorCss.includes('font-size: 6px')
    && !timelineConstructorCss.includes('font-size: 5px')
    && !timelineConstructorCss.includes('font-size: 4px')
    && timelineConstructorCss.includes('body.timeline-view-rooms .booking-block.is-room-timeline-activity-card.booking-block--micro')
    && timelineConstructorCss.includes('.mini-booking-text[data-max-token-length="5"]')
    && !timelineConstructorCss.includes('min-width: 124px'));
...
check('Sidebar collapse control is disabled and stale collapsed state is cleared', sidebarCode.includes('function _ensureSidebarCollapseButton') && sidebarCode.includes("document.getElementById('sidebarCollapseBtn')") && sidebarCode.includes('btn.remove()') && sidebarCode.includes("localStorage.removeItem('pzp_sidebar_collapsed')") && sidebarCode.includes('sidebar.classList.remove(\'collapsed\')') && sidebarCode.includes('function _setSidebarCollapsed') && responsiveCss.includes('v0.81.159: sidebar collapse mode is disabled') && !sidebarCode.includes("localStorage.setItem('pzp_sidebar_collapsed'"));
...
check('Sidebar collapse has no legacy or assistant path back into collapsed mode', sidebarCode.includes("sidebar.dataset.sidebarStateOwner = 'aurora'") && sidebarCode.includes('btn.remove()') && appCode.includes('collapseBtn.remove()') && fs.readFileSync(path.join(ROOT, 'js/assistant-foundation.js'), 'utf8').includes('disabled: true') && !appCode.includes('sidebarLegacyCollapseBound') && !appCode.includes("localStorage.setItem('pzp_sidebar_collapsed'") && !fs.readFileSync(path.join(ROOT, 'js/assistant-foundation.js'), 'utf8').includes("localStorage.setItem('pzp_sidebar_collapsed'"));
...
check('Dashboard opens a visible widget grid without drawing tools or a board canvas', dashboardHtml.includes('class="dashboard-grid dashboard-widget-grid" id="dashboardGrid"') && !dashboardHtml.includes('data-board-tool=') && !dashboardHtml.includes('id="dashboardBoardCanvas"') && !dashboardHtml.includes('DashboardPage.clearBoardContent()') && !dashboardHtml.includes('dashboard-showcase-strip'));
check('Dashboard retains legacy workspace config compatibility while rendering widgets', dashboardPageCode.includes("const DASHBOARD_WORKSPACE_MODE = 'workspace'") && dashboardRouteCode.includes("const DASHBOARD_WORKSPACE_MODE = 'workspace'") && dashboardPageCode.includes('renderFlatWidgetGrid(grid)') && !dashboardHtml.includes('data-dashboard-workspace="unified"') && !dashboardHtml.includes('Сцена + Board'));
...
check('Dashboard offers accessible widget handles and responsive drop feedback', dashboardPageCode.includes('class="widget-drag-handle"') && dashboardPageCode.includes('aria-label="Перемістити:') && dashboardCss.includes('.dashboard-grid.dashboard-widget-grid') && dashboardCss.includes('repeat(3, minmax(0, 1fr))') && dashboardCss.includes('repeat(2, minmax(0, 1fr))') && dashboardCss.includes('grid-template-columns: minmax(0, 1fr)') && dashboardCss.includes('touch-action: none') && dashboardCss.includes('min-height: 44px') && dashboardCss.includes('.widget-card.is-dragging') && dashboardCss.includes('.widget-card.is-drop-target') && dashboardCss.includes('.dashboard-widget-grid:not(.is-layout-editing) .widget-drag-handle'));
check('Dashboard work mode keeps one settings action while edit mode owns add and layout controls', dashboardHtml.includes('id="dashboardAddWidgetButton"') && dashboardHtml.includes('hidden>Додати віджет') && dashboardPageCode.includes('function syncDashboardLayoutModeControls') && dashboardPageCode.includes('addButton.hidden = !_layoutEditing') && dashboardPageCode.includes("customizeButton.textContent = _layoutEditing ? 'Готово' : 'Налаштувати'") && dashboardPageCode.includes("grid.className = 'dashboard-grid dashboard-widget-grid' + (_layoutEditing ? ' is-layout-editing' : '')") && dashboardCss.includes('.dashboard-add-widget-btn[hidden]'));
check('Dashboard explains widget movement in Ukrainian and announces layout saves', dashboardHtml.includes('<h1>Сьогодні</h1>') && dashboardHtml.includes('Перетягуйте віджети за ⠿') && dashboardHtml.includes('id="dashboardLayoutStatus" role="status" aria-live="polite"') && dashboardPageCode.includes('dashboardOrientationStatusLabel') && dashboardPageCode.includes('focus-task-status') && dashboardCss.includes('.dashboard-day-orientation-status') && !dashboardHtml.includes('dashboard-workspace-stage') && !dashboardHtml.includes('board-tool-rail') && !dashboardHtml.includes('data-board-tool-family='));
check('Dashboard preserves legacy board content tones without offering board presets', dashboardPageCode.includes('const BOARD_CONTENT_PRESETS') && dashboardPageCode.includes('function insertBoardContentPreset') && !dashboardHtml.includes('data-board-preset=') && dashboardCss.includes('[data-board-tone="production"]') && dashboardRouteCode.includes('BOARD_CONTENT_TONES') && dashboardRouteCode.includes('safe.tone = normalizeBoardTone'));
check('Dashboard has no visible board interaction mode switch', dashboardPageCode.includes("const BOARD_INTERACTION_MODE = 'unified'") && !dashboardHtml.includes('boardUnifiedModeLabel') && !dashboardHtml.includes('boardViewModeBtn') && !dashboardHtml.includes('boardEditModeBtn') && !dashboardHtml.includes('dashboardBoardModeBtn'));
...
const dashboardSettingsCode = dashboardPageCode.slice(dashboardPageCode.indexOf('function openSettings()'), dashboardPageCode.indexOf('async function saveSettings()'));
check('Dashboard settings offer widget selection without drawing controls', dashboardSettingsCode.includes('id="settingsWidgetList"') && !dashboardSettingsCode.includes('id="settingsBoardSnapToGrid"') && !dashboardSettingsCode.includes('id="settingsBoardShowGrid"') && !dashboardSettingsCode.includes('id="settingsBoardStrokeColor"') && !dashboardSettingsCode.includes('dashboard-settings-board-card'));
...
check('Dashboard still normalizes legacy note payload fields without an automatic board rewrite', dashboardPageCode.includes('item.noteText || item.content || item.body || item.label') && dashboardPageCode.includes('item.text ?? item.content ?? item.body ?? item.noteText ?? item.label') && dashboardRouteCode.includes('item.noteText || item.content || item.body || item.label') && dashboardRouteCode.includes('item.text ?? item.content ?? item.body ?? item.noteText ?? item.label') && !dashboardPageCode.includes("markBoardDirty('legacy-note-upgrade')"));
...
check('Timeline pinata hover and room labels expose operational numbers consistently',
    ...
    && timelinePresentationCode.includes("productCode = normalizePinataNumber(pinataNumber, options)")
    && timelinePresentationCode.includes('`Піньята парку ${pinataNumberDisplay(pinataNumber, options)}`')
...
check('Design file downloads use authenticated Blob transport', designsPageCode.includes('async function fetchDesignBlob') && designsPageCode.includes("headers: authHeaders(false)") && designsPageCode.includes('openTouchDownloadWindow') && designsPageCode.includes('finishBlobDownload') && !designsPageCode.includes('openSafeNewTab(href)'));
...
check('HR grouped IA keeps Pulse clean and vacancy workspace owns hiring surfaces',
    ...
    && htmlContains('hr.html', 'js/hr-pulse-switcher.js?v=0.82.19')
    ...
    && htmlContains('hr.html', 'js/staff-schedule-shell.js?v=0.82.19')
    && htmlContains('hr.html', 'js/staff-schedule-loader.js?v=0.82.19')
    && !htmlContains('hr.html', 'js/staff-page.js?v=0.82.19')
...
check('HR Account Center uses the dedicated effective-access workspace',
    htmlContains('hr.html', 'css/account-access-editor.css?v=0.82.19')
    && htmlContains('hr.html', 'js/account-access-editor.js?v=0.82.19')
...
check('Timeline sidebar collapse is disabled in the legacy shell', appCode.includes("sidebar.classList.remove('collapsed')") && appCode.includes("localStorage.removeItem('pzp_sidebar_collapsed')") && !appCode.includes("sidebar.classList.add('collapsed')") && !appCode.includes("sidebar.classList.toggle('collapsed')"));
...
check('Cashier payments production UI exposes the safe business/register selector and immutable fiscal/payment snapshot',
    cashierPaymentsHtml.includes('id="paymentBusinessContext"')
    && cashierPaymentsHtml.includes('id="paymentRegisterRoute"')
    && cashierPaymentsHtml.includes('name="routeOptionId"')
    && cashierPaymentsHtml.includes('id="paymentRouteHelp"')
...
check('Cashier frontend uses provider-aware readiness and server-backed recovery APIs',
    ...
    && cashierPaymentsJs.includes('loadRouteOptions')
    && cashierPaymentsJs.includes('/api/payments/catalog/routes')
    && cashierPaymentsJs.includes('X-Fiscal-Route-Option')
    && cashierPaymentsJs.includes('routeOptionId: PILOT_SCOPE.routeOptionId')
...
check('Cashier PIN UI keeps delegated enrollment separate from an owner-only test PIN check',
    ...
    && cashierPaymentsJs.includes("$('actionPinCheckForm')?.classList.toggle('hidden', !checkAvailable)")
    && cashierPaymentsJs.includes('Готовність продажів на це не впливає.')
    && cashierPaymentsJs.includes('PIN встановлює інший відповідальний; перевіряє його власник прив’язки після входу у CRM.')
    && cashierPaymentsJs.includes('state.actionPinCheckMessage')
    && cashierPaymentsJs.includes('if (contextKey !== interactionContextKey()) return;')
...
check('Cashier payments defines accessible dark warning and overflow containment surfaces',
    ...
    && cashierPaymentsJs.includes('selectedRoute()?.registerLabel')
    && cashierPaymentsHtml.includes('id="paymentRegisterRoute"')
