...
check('Room timeline operational lanes separate markers and activity cards', timelineCode.includes('function syncTimelineRoomOperationalLayout(lineGrid = null)') && timelineCode.includes("lineGrid.querySelectorAll('.timeline-room-service-marker')") && timelineCode.includes(".booking-block.is-room-timeline-activity-card:not(.status-hidden)") && timelineCode.includes('dataset.roomOperationalLane = String(lane)') && timelineCode.includes("style.setProperty('--timeline-room-lane-top'") && timelineCode.includes('dataset.roomActivityLane = String(lane)') && timelineCode.includes('syncTimelineRoomOperationalLayout(lineGrid);') && timelineConstructorCss.includes('.line-grid.has-timeline-room-operational-lanes') && timelineConstructorCss.includes('.timeline-line.has-timeline-room-operational-lanes') && timelineConstructorCss.includes('--timeline-room-operational-row-height') && timelineConstructorCss.includes('--timeline-room-activity-card-height: 72px') && timelineConstructorCss.includes('height: var(--timeline-room-activity-card-height)'));
