...
<section class="diploma-page" style="--paper:${palette.paper};--ink:${palette.ink};--muted:${palette.muted};--gold:${palette.gold};--gold-soft:${palette.goldSoft};--accent:${palette.accent};">
    <svg class="diploma-frame" viewBox="0 0 790 1120" aria-hidden="true" focusable="false">
        <defs>
            ...
            <linearGradient id="diplomaBrand" x1="0" x2="1">
                <stop offset="0" stop-color="#21b7d7"/>
                <stop offset="0.26" stop-color="#1678bd"/>
                <stop offset="0.5" stop-color="#0a7439"/>
                <stop offset="0.74" stop-color="#d61f72"/>
                <stop offset="1" stop-color="#f5a51a"/>
            </linearGradient>
            <pattern id="diplomaMicroDots" width="24" height="24" patternUnits="userSpaceOnUse">
                <circle cx="4" cy="4" r="1.25" fill="#1678bd" opacity="0.12"/>
            </pattern>
        </defs>
        <rect x="25" y="25" width="740" height="1070" rx="4" fill="none" stroke="url(#diplomaBrand)" stroke-width="7"/>
        <rect x="43" y="43" width="704" height="1034" rx="2" fill="none" stroke="${escHtml(palette.gold)}" stroke-width="2.4" opacity="0.86"/>
        <rect x="62" y="62" width="666" height="996" rx="2" fill="url(#diplomaMicroDots)" opacity="0.42"/>
        <path d="M88 86 H702" stroke="url(#diplomaBrand)" stroke-width="4" opacity="0.76"/>
        <path d="M88 103 H702" stroke="${escHtml(palette.gold)}" stroke-width="1.5" opacity="0.45"/>
        <path d="M124 158 C78 216 82 306 139 360 M666 158 C712 216 708 306 651 360" fill="none" stroke="#1678bd" stroke-width="4.2" stroke-linecap="round" opacity="0.38"/>
        ${Array.from({ length: 8 }).map((_, i) => {
            const y = 184 + i * 25;
            const r = 6 + (i % 2);
            const color = i % 3 === 0 ? '#7fb539' : (i % 3 === 1 ? '#21b7d7' : '#f5a51a');
            return `<ellipse cx="${120 - i * 3}" cy="${y}" rx="${r}" ry="${r + 4}" fill="${color}" opacity="0.32" transform="rotate(${-32 + i * 4} ${120 - i * 3} ${y})"/>
                    <ellipse cx="${670 + i * 3}" cy="${y}" rx="${r}" ry="${r + 4}" fill="${color}" opacity="0.32" transform="rotate(${32 - i * 4} ${670 + i * 3} ${y})"/>`;
        }).join('')}
        <circle cx="70" cy="70" r="16" fill="#21b7d7" opacity="0.72"/>
        <circle cx="720" cy="70" r="16" fill="#7fb539" opacity="0.72"/>
        <circle cx="70" cy="1050" r="16" fill="#d61f72" opacity="0.72"/>
        <circle cx="720" cy="1050" r="16" fill="#f5a51a" opacity="0.72"/>
        <path d="M258 906 Q395 974 532 906" fill="none" stroke="url(#diplomaBrand)" stroke-width="3" opacity="0.5"/>
        <path d="M305 930 Q395 970 485 930" fill="none" stroke="${escHtml(palette.gold)}" stroke-width="1.6" opacity="0.42"/>
    </svg>
...
.diploma-page { width: 210mm; height: 297mm; margin: 14px auto; position: relative; overflow: hidden; background: radial-gradient(circle at 50% 18%, rgba(255,255,255,0.84), transparent 34%), linear-gradient(145deg, var(--paper), #f9eec7); color: var(--ink); page-break-after: always; break-after: page; box-shadow: 0 22px 70px rgba(23,59,99,0.18); -webkit-print-color-adjust: exact; print-color-adjust: exact; }
.diploma-frame { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
.diploma-content { position: relative; z-index: 2; height: 100%; padding: 42mm 24mm 16mm; display: flex; flex-direction: column; align-items: center; text-align: center; }
.diploma-content h1 { margin: 13mm 0 6mm; font-size: 42pt; line-height: 0.98; color: var(--accent); font-weight: 800; letter-spacing: 0.015em; text-transform: uppercase; }
.diploma-subtitle { max-width: 145mm; margin: 0 auto 13mm; font-size: 13pt; color: var(--muted); line-height: 1.5; }
.diploma-awarded { font-size: 12.5pt; letter-spacing: 0.12em; text-transform: uppercase; color: #1678bd; margin-bottom: 5mm; font-weight: 800; }
.diploma-name { width: 100%; max-width: 158mm; padding: 5mm 6mm 6mm; border-top: 2.2px solid rgba(22,120,189,0.42); border-bottom: 2.2px solid rgba(214,31,114,0.36); font-size: 42pt; line-height: 1.05; color: var(--ink); font-weight: 800; text-wrap: balance; }
...
.diploma-wish { max-width: 150mm; min-height: 42mm; margin: 13mm auto 0; font-size: 15pt; line-height: 1.55; color: var(--ink); display: flex; align-items: center; justify-content: center; }
.diploma-official-zone { margin-top: auto; margin-bottom: 2mm; display: flex; flex-direction: column; align-items: center; gap: 4mm; }
.diploma-park-seal { width: 31mm; height: 31mm; border-radius: 50%; display: grid; place-items: center; padding: 2.1mm; border: 2.5px double var(--gold); background: rgba(255,255,255,0.48); box-shadow: 0 0 0 1.4mm rgba(22,120,189,0.08), 0 0 0 2.2mm rgba(245,165,26,0.12); overflow: hidden; }
.diploma-park-seal img { width: 100%; height: 100%; object-fit: contain; border-radius: 50%; display: block; }
.diploma-date { min-width: 48mm; padding: 2mm 7mm; border-top: 1.2px solid rgba(22,120,189,0.34); border-bottom: 1.2px solid rgba(214,31,114,0.26); color: #173b63; font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; font-size: 10.8pt; font-weight: 900; letter-spacing: 0.03em; background: rgba(255,255,255,0.24); }
