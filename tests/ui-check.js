...
check('Sidebar identity card has real compact time weather and currency signals', sidebarCode.includes('sidebarIdentityTime') && sidebarCode.includes('sidebarIdentityWeather') && sidebarCode.includes('sidebarIdentityCurrency') && sidebarCode.includes('/api/dashboard/widgets/weather') && sidebarCode.includes('/api/dashboard/widgets/currency') && sidebarCode.includes('Europe/Kyiv') && sidebarAuroraCss.includes('v0.56.3: sidebar identity status card polish') && sidebarAuroraCss.includes('.sidebar-identity-meta') && sidebarAuroraCss.includes('grid-template-columns: repeat(3, minmax(0, 1fr)) !important'));
...
check('Sidebar identity card places USD time weather as a right-side signal rail', sidebarAuroraCss.includes('v0.57.8: sidebar identity card right-side signal rail') && sidebarAuroraCss.includes('"identity signals"') && sidebarAuroraCss.includes('"summary signals"') && sidebarAuroraCss.includes('grid-area: signals !important') && sidebarAuroraCss.includes('grid-auto-rows: minmax(22px, 1fr) !important') && sidebarAuroraCss.includes('.sidebar-identity-meta::before'));
check('Sidebar identity card opens richer clickable signal details', sidebarCode.includes('function _fetchSidebarWeatherFallback') && sidebarCode.includes('function _showSidebarIdentityDetail') && sidebarCode.includes('data-sidebar-stop-profile="true"') && sidebarAuroraCss.includes('v0.57.14: clickable profile signals') && sidebarAuroraCss.includes('.sidebar-identity-detail-panel') && sidebarAuroraCss.includes('#sidebarIdentityHealthLabel') && sidebarAuroraCss.includes('-webkit-line-clamp: 2'));
