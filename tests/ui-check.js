...
checkPage('art-director.html', (doc, html) => {
    ...
    check('HR structure route sanitizes structured node payloads', hrRouteCode.includes('sanitizeCompanyStructureNodes') && hrRouteCode.includes('COMPANY_STRUCTURE_ALLOWED_TONES') && hrRouteCode.includes('schemaVersion: COMPANY_STRUCTURE_SCHEMA_VERSION'));
...
checkPage('customers.html', (doc, html) => {
    ...
    check('Customers expose durable CRM business context switch', !!doc.getElementById('customerBusinessContext') && htmlContains('js/api.js', 'window.CrmBusinessContext') && htmlContains('js/customers-page.js', 'initCustomerBusinessContext') && htmlContains('js/customers-page.js', 'customerApiUrl'));
});
...
checkPage('leads.html', (doc, html) => {
    ...
    check('Leads expose durable CRM business context switch', !!doc.getElementById('leadBusinessContext') && htmlContains('js/leads-page.js', 'initLeadBusinessContext') && htmlContains('js/leads-page.js', 'leadApiUrl'));
});
...
check('Profile work header uses spacious responsive desktop polish', profilePageHtml.includes('grid-template-columns: minmax(360px, 1.1fr) minmax(620px, 1.55fr);') && profilePageHtml.includes('.profile-work-stat.wide') && profilePageHtml.includes('grid-column: auto;') && profilePageHtml.includes('.profile-work-stat:hover') && profilePageHtml.includes('@media (max-width: 1320px)'));
...
check('Profile work overview uses controlled desktop section spans', profilePageHtml.includes('grid-template-columns: repeat(12, minmax(0, 1fr));') && profilePageHtml.includes('.profile-work-panel-tasks') && profilePageHtml.includes('.profile-work-panel-risks') && profilePageHtml.includes('.profile-work-panel-progress') && profilePageHtml.includes('.profile-work-panel-activity'));
...
check('Profile overview removes vague work-state copy and ships next-shift cockpit data', profileCode.includes('Особистий фокус') && profileCode.includes('Наступна зміна') && !profileCode.includes('<h2>Робочий стан</h2>') && authRouteCode.includes('profile_cockpit_widgets') && authRouteCode.includes('nextShift') && authRouteCode.includes('COUNT(*)::int AS total'));
check('Profile cabinet task actions use delegated buttons and canonical task deeplink', profileCode.includes('function handleCabinetTaskActionClick') && profileCode.includes('data-cabinet-task-action="done"') && profileCode.includes('data-cabinet-task-action="snooze"') && profileCode.includes('data-cabinet-task-action="open"') && profileCode.includes('/tasks?view=my&open=') && profileCode.includes('/tasks/${id}/complete') && profileCode.includes('/tasks/${id}/snooze') && !profileCode.includes('onclick="setCabinetTaskStatus') && !profileCode.includes("'/tasks?task="));
...
check('Profile cabinet task actions expose undo, snooze menu, and due badges', profileCode.includes('function showCabinetTaskUndoToast') && profileCode.includes('data-cabinet-task-action="snooze-menu"') && profileCode.includes("['1440', 'Завтра']") && profileCode.includes('data-task-due-state') && profileCode.includes('function renderCabinetTaskActionLegend') && profileCode.includes('window.prompt(') && profileCode.includes('відкласти задачу?') && profilePagesCss.includes('.cabinet-snooze-menu') && profilePagesCss.includes('.cabinet-task-undo-toast') && profilePagesCss.includes('.cabinet-task-due-badge--overdue') && profilePagesCss.includes('.cabinet-action-legend'));
check('Profile overdue due badge opens reschedule choices and respects permission control', profileCode.includes('data-cabinet-task-action="reschedule-overdue-menu"') && profileCode.includes('data-cabinet-task-action="reschedule-overdue"') && profileCode.includes("['day_after', 'Післязавтра']") && profileCode.includes("sourceSurface: 'profile_my_cabinet_overdue_badge'") && profileCode.includes('cabinetTaskAllowReschedule') && profilePagesCss.includes('.cabinet-reschedule-menu'));
check('Profile cabinet tasker has truthful segment counts and composer', profileCode.includes('const CABINET_TASK_SEGMENTS') && profileCode.includes('function cabinetTaskMatchesSegment') && profileCode.includes('function cabinetSegmentCounts') && profileCode.includes('function renderCabinetTaskComposer') && profileCode.includes('function isCabinetPersonalTask') && profileCode.includes("visibility === 'me_only'") && profileCode.includes("category === 'improvement'") && profileCode.includes('data-cabinet-active-segment') && profileCode.includes('data-cabinet-due-preset') && profilePagesCss.includes('.cabinet-task-composer') && profilePagesCss.includes('.cabinet-list-head') && profilePagesCss.includes('.cabinet-task-priority'));
check('Profile task creation reuses canonical TaskCreate adapter', profilePageHtml.includes('js/task-create.js') && fs.readFileSync(path.join(ROOT, 'tasks.html'), 'utf8').includes('js/task-create.js') && profileCode.includes('window.TaskCreate.buildPayload') && profileCode.includes('window.TaskCreate.createTask') && taskCreateCode.includes('window.TaskCreate') && taskCreateCode.includes('buildPayload') && taskCreateCode.includes('createTask') && taskCreateCode.includes('/tasks'));
check('Profile tasker completion replaces waiting primary segment and gates report-backed completion', profileCode.includes("{ id: 'actionable', label: 'Виконати'") && !profileCode.includes("{ id: 'waiting', label: 'Чекаю'") && profileCode.includes('cabinetTaskReportBadge') && profileCode.includes('cabinetTaskReportRequired') && profileCode.includes('window.TaskReportGate.openReportModal') && taskCreateCode.includes('window.TaskReportGate') && taskCreateCode.includes('/completion-report') && profilePagesCss.includes('.cabinet-task-report-badge'));
...
check('Timeline button defaults to Park while MD context stays creator-only', sidebarCode.includes("label: 'Таймлайн'") && sidebarCode.includes("href: '/maysternya-doli'") && sidebarCode.includes("quickAccessOnly: true") && featureRegistryCode.includes("title: 'Таймлайн'") && featureRegistryCode.includes("title: 'Таймлайн МД'") && searchCode.includes("label: 'Таймлайн'") && htmlContains('js/timeline-context.js', "switchLabel: 'Таймлайн ПАРК'") && htmlContains('js/timeline-context.js', 'showAfisha: false') && htmlContains('js/timeline-context.js', "settings: ['creator']") && htmlContains('services/timelineContext.js', "return userRoles(user).includes('creator')") && authCode.includes("quickAccess: ['/dashboard', '/', '/certificates/new', '/center', '/tasks', '/chat']") && authCode.includes("normalized !== '/maysternya-doli'") && htmlContains('js/timeline.js', 'function timelineShouldRenderAfisha') && htmlContains('js/timeline.js', 'showAfisha ? apiGetAfishaByDate') && htmlContains('js/timeline.js', 'const allAfisha = showAfisha ? (afishaEvents || []) : []') && htmlContains('js/timeline.js', 'function normalizeTimelineLinesForContext') && htmlContains('routes/lines.js', "name: 'Олександр'"));
check('Global search preserves active business context for CRM results', searchCode.includes('CrmBusinessContext.apiUrl') && searchRoutes.includes('requireBusinessContext') && searchRoutes.includes('COALESCE(c.business_context') && searchRoutes.includes("COALESCE(business_context, $2) = $2") && searchRoutes.includes("'/maysternya-doli'") && searchRoutes.includes('businessContext'));
check('Timeline has business switch and visual element visibility constructor', htmlContains('index.html', 'js/timeline-visibility.js') && timelineVisibilityCode.includes('TIMELINE_VISIBILITY_ELEMENTS') && timelineVisibilityCode.includes('timelineBusinessSelect') && timelineVisibilityCode.includes('timelineConstructorBtn') && timelineVisibilityCode.includes('bindConstructorButton') && timelineVisibilityCode.includes("canUseAction('settings'") && timelineVisibilityCode.includes("const STORAGE_NAME = 'timeline_element_visibility'") && timelineVisibilityCode.includes('localStorage.getItem(storageKey())') && timelineVisibilityCode.includes("/settings/timeline-visibility") && timelineVisibilityCode.includes('timelineScale') && timelineVisibilityCode.includes('bookingClose') && timelineVisibilityCode.includes('timeline-hidden-by-config') && timelineConstructorCss.includes('.timeline-permission-hidden') && htmlContains('js/timeline-context.js', 'defaultHiddenElements') && authCode.includes('TimelineVisibility.refreshAccess') && fs.readFileSync(path.join(ROOT, 'js/config.js'), 'utf8').includes("app:user-changed") && timelineConstructorCss.includes('.timeline-constructor-panel') && timelineConstructorCss.includes('.timeline-constructor-btn') && timelineConstructorCss.includes('.timeline-visibility-chip'));
check('Timeline room load drawer is dark-themeable and closable', featuresCss.includes('html[data-theme="dark"] .room-load-panel') && featuresCss.includes('.room-load-close-label') && timelineCode.includes('closeRoomLoadPanel') && timelineCode.includes("event.key === 'Escape'") && timelineCode.includes("btn.setAttribute('aria-expanded', 'false')"));
check('Timeline banquet links use durable API-backed connector model', htmlContains('db/migrations/216_booking_banquet_links.sql', 'CREATE TABLE IF NOT EXISTS booking_banquet_links') && htmlContains('routes/bookings.js', "router.post('/:id/banquet-links'") && htmlContains('routes/bookings.js', "router.delete('/:id/banquet-links/:targetId'") && htmlContains('services/booking.js', 'banquetLinks: Array.isArray(row.banquet_links)') && timelineCode.includes('booking-banquet-link-handle') && timelineCode.includes('renderBanquetLinksOverlay') && timelineCode.includes('apiCreateBookingBanquetLink') && timelineCode.includes('removeBookingBanquetLink'));
check('Timeline banquet connector visual layer is non-blocking and dark-themeable', timelineConstructorCss.includes('.timeline-banquet-link-layer') && timelineConstructorCss.includes('pointer-events: none') && timelineConstructorCss.includes('.timeline-banquet-link-path') && timelineConstructorCss.includes('.booking-banquet-link-handle') && timelineConstructorCss.includes('body.banquet-linking-active') && timelineConstructorCss.includes('html[data-theme="dark"] .booking-banquet-links-detail'));
check('Timeline toolbar controls share one CRM glass language', darkModeCss.includes('v0.63.14: Timeline toolbar uses one CRM glass control language') && darkModeCss.includes('.timeline-dashboard-page .control-panel') && darkModeCss.includes('.timeline-dashboard-page :where(.status-filter-btn, .period-btn, .zoom-btn)') && darkModeCss.includes('.timeline-dashboard-page :where(.btn-nav, .btn-today, .toggle-mini, .btn-room-load, .btn-export, .btn-product-sales, .btn-menu-toggle, .btn-undo)'));
...
check('Task quick widgets split completed and active today/undated counts', profileCode.includes('function cabinetTaskQuickCounts') && profileCode.includes('cabinet-quick-split') && profileCode.includes('cabinet-quick-half--completed') && profileCode.includes('cabinet-quick-half--remaining') && profilePagesCss.includes('.cabinet-quick-divider') && sidebarCode.includes('/api/tasks/my-cabinet') && sidebarCode.includes('focusChipTasksDoneValue') && sidebarCode.includes('focus-chip-task-split') && sidebarAuroraCss.includes('.focus-chip-task-divider') && !sidebarCode.includes("Number(tasks.assigned || 0) + Number(tasks.in_progress || 0)"));
...
check('Sidebar removes duplicated day menu and keeps Additional as the only quick CRM section with certificate creation shortcuts', !sidebarCode.includes('TODAY_MENU_HREFS') && !sidebarCode.includes('sidebar-today-menu-grid') && sidebarCode.includes('_removeSidebarTodayDock') && sidebarCode.includes("const EXTRA_MENU_HREFS = ['/dashboard', '/afisha', '/', '/certificates/new', '/certificates/batch', '/tasks', '/chat']") && sidebarCode.includes("href: '/certificates'") && sidebarCode.includes("href: '/certificates/new'") && sidebarCode.includes('quickAccessOnly: true') && !sidebarCode.includes("href: '#certificates'"));
check('Role quick access orders animator Afisha before timeline and gives managers certificate issuance', authCode.includes("quickAccess: ['/afisha', '/', '/tasks', '/programs', '/chat', '/training']") && authCode.includes("manager: {\n        startPage: '/dashboard'") && authCode.includes("quickAccess: ['/dashboard', '/certificates/new', '/', '/sales-funnel', '/tasks', '/chat']") && authCode.includes("senior_manager: {\n        startPage: '/dashboard'") && authCode.includes("vice_director: {\n        startPage: '/dashboard'") && authCode.includes("director: {\n        startPage: '/dashboard'") && sidebarCode.includes("timeline:       [..._ADMIN_UP, 'reception', 'animator'") && featureRegistryCode.includes("id: 'afisha.events'") && featureRegistryCode.includes("href: '/afisha'"));
...
check('Sidebar quick access header replaces bulky Additional editor button', sidebarCode.includes('Швидкий доступ') && sidebarCode.includes('sidebar-design-extras-gear') && sidebarCode.includes('Редагувати швидкий доступ') && sidebarCode.includes('Сторінки швидкого доступу') && sidebarAuroraCss.includes('v0.57.1: Quick Access submenu polish') && sidebarAuroraCss.includes('grid-template-columns: minmax(0, 1fr) 34px !important') && sidebarAuroraCss.includes('.sidebar-design-extras-manage-text') && sidebarAuroraCss.includes('sidebarQuickGearSpin') && sidebarAuroraCss.includes('.sidebar-design-extras:not(.is-collapsed) .sidebar-design-extra-list::before'));
...
check('Sidebar identity card places USD and time as equal compact chips', sidebarAuroraCss.includes('v0.58.0: enterprise sidebar navigation redesign') && sidebarAuroraCss.includes('.sidebar-identity-meta-item[data-sidebar-meta="currency"]') && sidebarAuroraCss.includes('order: 1 !important') && sidebarAuroraCss.includes('.sidebar-identity-meta-item[data-sidebar-meta="time"]') && sidebarAuroraCss.includes('order: 2 !important') && sidebarAuroraCss.includes('.sidebar-identity-meta-item[data-sidebar-meta="weather"]') && sidebarAuroraCss.includes('display: none !important'));
check('Sidebar profile summary cleanup removes oval, de-pills role, and keeps three metrics inline', sidebarCode.includes('const label = _sidebarRoleLabel(raw);') && !sidebarCode.includes('id="sidebarIdentityHealth"') && !sidebarCode.includes('sidebar-identity-health-dot') && sidebarAuroraCss.includes('v0.60.37: profile summary card cleanup') && sidebarAuroraCss.includes('.sidebar-identity-summary::before') && sidebarAuroraCss.includes('display: none !important') && sidebarAuroraCss.includes('border-radius: 0 !important') && sidebarAuroraCss.includes('border-left: 1px solid color-mix') && sidebarAuroraCss.includes('grid-template-columns: repeat(3, minmax(0, 1fr)) !important') && sidebarAuroraCss.includes('.sidebar-identity-meta-item[data-sidebar-meta="date"]') && sidebarAuroraCss.includes('grid-column: auto !important'));
...
check('Sidebar enterprise redesign exposes component contracts, metric tones, ARIA and denser target widths', sidebarCode.includes('SIDEBAR_COMPONENTS') && sidebarCode.includes('SidebarShell') && sidebarCode.includes('UserSummaryCard') && sidebarCode.includes('function getMetricTone') && sidebarCode.includes("case 'tasks'") && sidebarCode.includes("case 'alerts'") && sidebarCode.includes("case 'leads'") && sidebarCode.includes('aria-expanded=') && sidebarCode.includes('aria-current="page"') && sidebarAuroraCss.includes('--eg-claude-sidebar-w: clamp(224px, 15vw, 232px)') && sidebarAuroraCss.includes('--eg-sidebar-collapsed-w: 80px') && sidebarAuroraCss.includes('width: min(92vw, 360px)'));
...
check('Training page preserves the shared shell instead of entering isolated mode', trainingPageCode.includes('function restoreTrainingShellVisibility') && trainingPageCode.includes('training-shell-ready') && trainingPageCode.includes("window.addEventListener('pageshow'") && trainingPageCode.includes('apiVerifyToken') && layoutCss.includes('v0.59.2: Training is a normal CRM page') && layoutCss.includes('body.training-shell-ready #mainApp') && layoutCss.includes('body.training-shell-ready:not(.auth-screen) .sidebar-nav') && layoutCss.includes('body.training-shell-ready:not(.auth-screen) #logoutBtn'));
...
check('Shared assistant rail uses one canonical avatar presence path', assistantRailCode.includes('assistant-rail-avatar-core') && assistantRailCode.includes('assistant-rail-avatar-screen') && assistantRailCode.includes('assistant-rail-avatar-eye--left') && assistantRailCode.includes('assistant-rail-avatar-mouth') && !assistantRailCode.includes('assistant-rail-dot-cloud') && !assistantRailCode.includes('assistant-rail-face') && !assistantRailCode.includes('assistant-rail-glasses') && assistantRailCss.includes('v0.60.2: assistant full rethink') && assistantRailCss.includes('.assistant-rail-avatar-core') && assistantRailCss.includes('.assistant-rail-avatar-screen') && assistantRailCss.includes('[data-ai-state="thinking"] .assistant-rail-avatar-core') && assistantRailCss.includes('[data-ai-state="listening"] .assistant-rail-avatar-core') && assistantRailCss.includes('[data-mode="speaking"] .assistant-rail-avatar-mouth') && assistantRailCss.includes('[data-ai-state="error"] .assistant-rail-avatar-core'));
...
check('Customers CRUD uses Postgres by default and keeps Supabase customers table opt-in', customersRouteCode.includes('CUSTOMERS_SUPABASE_ENABLED') && customersRouteCode.includes('function getCustomerSupabase') && customersRouteCode.includes('const sb = getCustomerSupabase();') && customersRouteCode.includes("router.post('/migrate-to-supabase'") && customersRouteCode.includes('const sb = getSupabase();'));
...
check('Timeline compact mode fits the visible screen and syncs the toggle state', uiCode.includes('function _timelineFitCellWidth') && uiCode.includes('container.dataset.fitScreen') && uiCode.includes('event?.target?.checked') && htmlContains('js/app.js', 'compactToggle.checked = AppState.compactMode') && htmlContains('css/timeline.css', 'v0.56.5: timeline compact fit-screen density') && htmlContains('css/controls.css', 'keep compact zoom modes genuinely compact'));
