...
describe('Communication Send Truth v1', () => {
    ...
    it('renders unique social channel identity in every Omni dialog row', () => {
        ...
        assert.match(omniHtml, /const CHANNEL_ASSET_PATHS/);
        assert.match(omniHtml, /telegram:\s*'\/images\/omni-icons\/telegram\.png'/);
        assert.match(omniHtml, /report_bot:\s*'\/images\/omni-icons\/telegram\.png'/);
        assert.match(omniHtml, /instagram:\s*'\/images\/omni-icons\/instagram\.png'/);
        assert.match(omniHtml, /facebook:\s*'\/images\/omni-icons\/facebook\.png'/);
        assert.match(omniHtml, /viber:\s*'\/images\/omni-icons\/viber\.png'/);
        assert.match(omniHtml, /whatsapp:\s*'\/images\/omni-icons\/whatsapp\.png'/);
        assert.match(omniHtml, /function getChannelAsset/);
        assert.match(omniHtml, /provider\s*\|\|\s*''\)\.toLowerCase\(\)\.trim\(\)\s*===\s*'flysms'/);
        assert.match(omniHtml, /'\/images\/omni-icons\/sms-fly\.png'/);
        assert.match(omniHtml, /function renderChannelMark/);
        assert.match(omniHtml, /alt="" aria-hidden="true" decoding="async"/);
        assert.match(omniHtml, /omni-channel-dot--logo/);
        assert.match(omniHtml, /omni-channel-mark--sms-fly/);
        assert.doesNotMatch(omniHtml, /onerror\s*=/i, 'channel logos do not retry failed image requests');
        assert.match(omniHtml, /function renderChannelDot/);
        assert.match(omniHtml, /function renderChannelBadge/);
        assert.match(omniHtml, /renderChannelDot\(c\.channel,\s*c\.provider\)/);
        assert.doesNotMatch(omniHtml, /renderChannelBadge\(c\.channel,\s*'list'\)/, 'list uses one channel marker beside the avatar');
        assert.match(omniHtml, /renderChannelBadge\(conv\.channel,\s*'header',\s*conv\.provider\)/);
        assert.match(omniHtml, /renderChannelMark\(acc\.channel,\s*acc\.provider\)/);
        assert.match(omniHtml, /setConnectionTitle\(acc\.channel,\s*title,\s*provider\)/);
