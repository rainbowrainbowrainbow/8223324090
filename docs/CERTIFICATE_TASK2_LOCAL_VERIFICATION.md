...
- Read-only перевірка наявного локального тестового акаунта: активний, QA-позначений, без staff profile, з чинним членством Парку і роллю `senior_manager`. Чинний `requireRole` дозволяє цій ролі видачу через legacy-розширення `admin`, а redemption policy — пряме погашення. Виявлено й виправлено вужчий список ролей у QA operator preflight, який помилково відхиляв цей акаунт. Повторний `--mode plan` у явній `READ ONLY` транзакції пройшов: один точний акаунт, ізоляція підтверджена, відкритих certificate QA-runs — 0, строк плану 30 хв. Ролі, членство та credentials не змінювали.
