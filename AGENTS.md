...
- My Day AI rollout evidence is operator-run, not automatic CI: use `npm run task-ai-rollout-report -- --stdin --database --format markdown` or `--events-file <railway-jsonl>` with a dedicated read-only `TASK_AI_ROLLOUT_DATABASE_URL`. The report passes volume evidence with 30 successful proposals or time evidence with 24 stable timestamped hours; DB consistency and provider-error gates still must pass. The report must remain redacted and must not use `DATABASE_URL` as a fallback.
...
- `--recover-missing-live-metadata-commit <40-char-sha>` is emergency-only for incomplete live metadata after the previous live/marker SHA was independently confirmed. Do not add it to npm wrappers or routine release commands; normal deploys must not use it.
