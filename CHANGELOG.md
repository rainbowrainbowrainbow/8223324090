...
- **Вкладку оголошень перероблено як робочий екран** - додано command header, статистику, нові картки оголошень, статуси голосу, програвання і delivery-метадані.
- **Кнопка "Нове оголошення" створює реальні записи** - frontend тепер відправляє `text_content`, `announcement_type`, `duration_seconds` і числовий `priority`, а API приймає також legacy alias-и для сумісності.
- **TTS має реальну панель підключення** - у вкладці є форма для Kie.ai/ElevenLabs API key, моделі, голосу, мови, тесту підключення і швидкої генерації голосу в бібліотеку.
- **Оголошення можна озвучувати з картки** - додано генерацію TTS для конкретного оголошення, polling task status і apply-flow, який привʼязує `voice_url` до запису.
- **Guardrails** - UI smoke перевіряє новий announcement surface, TTS settings panel, правильний API payload і backend `sound_tts_config` contract.

---

## v0.63.45 - CRM 63.45: порожній стан сертифікатів

### Сертифікати / registry empty state / 25.05.2026 [codex]
- **Порожній стан реєстру вирівняно** - zero-results branch у реєстрі сертифікатів тепер має власний layout-контракт для заголовка, повідомлення і CTA.
- **CTA лишився прямим route link** - кнопка веде на `/certificates/new`, але отримала стабільне розміщення під текстом і читабельний focus-visible стан.
- **Responsive і dark mode уточнено** - додано вузько scoped правила для mobile width та темної теми без зміни API, фільтрів або логіки сертифікатів.
- **UI guardrail** - статичний UI smoke перевіряє dedicated empty-state markup, dark-mode text і focus contract.
