...
checkPage('certificates.html', (doc, html) => {
    ...
    check('Certificates nested routes load shared shell assets from root', ['/css/base.css', '/css/sidebar-aurora.css', '/js/auth.js', '/js/components/sidebar.js', '/js/certificates-page.js', '/images/gear-logo.svg'].every(token => html.includes(token)) && !html.includes('href="css/') && !html.includes('src="js/') && !html.includes('src="images/gear-logo.svg"'));
...
check('Profile work header uses compact single-row metric polish on desktop', profilePageHtml.includes('grid-template-columns: minmax(280px, 0.92fr) minmax(520px, 1fr);') && profilePageHtml.includes('.profile-work-stat.wide') && profilePageHtml.includes('grid-column: auto;') && profilePageHtml.includes('.profile-work-stat:hover') && profilePageHtml.includes('@media (max-width: 980px)'));
check('Profile work shell is anchored near the CRM content edge on wide screens', profilePageHtml.includes('max-width: min(1360px, calc(100vw - 320px));') && profilePageHtml.includes('margin-left: 0;') && profilePageHtml.includes('margin-right: auto;'));
...
check('Profile tasker copy avoids canonical/cockpit leaks', profileCode.includes('Додати в мій робочий простір') && profileCode.includes('Задачу створено в основних задачах') && profileCode.includes('Панель продуктивності') && !profileCode.includes('Додати в мій cockpit') && !profileCode.includes('canonical Tasks') && !profileCode.includes('Productivity cockpit'));
...
check('Profile settings exposes personal account security controls', profileCode.includes("apiGet('/auth/security')") && profileCode.includes('function renderProfileSecurityPanel') && profileCode.includes('function openProfilePasswordModal') && profileCode.includes('/auth/security/revoke-sessions') && profilePageHtml.includes('.profile-security-panel') && profilePageHtml.includes('.profile-security-card'));
...
check('Dashboard workspace exposes Ukrainian tool labels, snap presets and compact top options', dashboardPageCode.includes('function setBoardSnapMode') && dashboardPageCode.includes('normalizeBoardSnapMode') && dashboardPageCode.includes('BOARD_SNAP_LABELS') && dashboardPageCode.includes('Планування композиції') && dashboardPageCode.includes('BOARD_PLANNING_ZONES') && dashboardHtml.includes('Спокійна сцена + режим планування') && dashboardHtml.includes('data-board-tool="space"') && !dashboardHtml.includes('Workspace</button>') && !dashboardHtml.includes('Reset view') && dashboardCss.includes('--workspace-selection-ring') && dashboardCss.includes('.board-tool-snap-presets') && dashboardCss.includes('.dashboard-workspace-toolbar .board-tool-options') && dashboardCss.includes('.dashboard-planner-zone') && dashboardPageCode.includes('data-workspace-module="true"'));
check('Dashboard board preserves drawing, connector, and active-tool state through frontend/backend config', dashboardPageCode.includes('normalizeBoardStroke') && dashboardPageCode.includes('drawings: drawingsRaw') && dashboardPageCode.includes('connectors: connectorsRaw') && dashboardRouteCode.includes('sanitizeBoardStroke') && dashboardRouteCode.includes('sanitizeBoardConnector') && dashboardRouteCode.includes('connectors,') && dashboardRouteCode.includes('activeTool: normalizeBoardTool'));
