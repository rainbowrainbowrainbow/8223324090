...
check('HR exposes account center with account creation and password controls', htmlContains('hr.html', 'data-tab="accounts"') && htmlContains('hr.html', 'accountCenterList') && htmlContains('hr.html', 'accountCreateBtn') && hrCode.includes('function loadAccountCenter') && hrCode.includes('function canManageAccountSecurity') && hrCode.includes('openAccountCreateModal') && hrCode.includes('function openAccountPasswordModal') && hrCode.includes('/api/users/${encodeURIComponent(userId)}/reset-password') && hrCode.includes('function deactivateKarinaAccounts') && htmlContains('routes/users.js', 'ACCOUNT_MANAGER_ROLES'));
