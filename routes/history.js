/**
 * routes/history.js — History log endpoints
 */
...
router.get('/', async (req, res) => {
    try {
        ...
        if (user) { conditions.push(`username = $${paramIdx++}`); params.push(user); }
        if (from) { conditions.push(`created_at >= $${paramIdx++}`); params.push(from); }
        if (to) { conditions.push(`created_at < ($${paramIdx++})::date + 1`); params.push(to); }
        if (search) {
