...
check('Dashboard workspace uses grouped pro-style tool families', dashboardHtml.includes('board-pro-palette') && ['interaction', 'navigate', 'insert', 'draw', 'shape', 'connect', 'templates', 'actions'].every(family => dashboardHtml.includes(`data-board-tool-family="${family}"`)) && dashboardCss.includes('.board-tool-family::before') && dashboardCss.includes('.dashboard-btn[data-board-tool="rect"]::before'));
check('Dashboard workspace exposes snap presets and clean module selection tokens', dashboardPageCode.includes('function setBoardSnapMode') && dashboardPageCode.includes('normalizeBoardSnapMode') && dashboardCss.includes('--workspace-selection-ring') && dashboardCss.includes('.board-tool-snap-presets') && dashboardPageCode.includes('data-workspace-module="true"'));
