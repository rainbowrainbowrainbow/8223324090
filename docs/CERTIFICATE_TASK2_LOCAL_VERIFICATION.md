...
Актуальний статус на 2026-09-26: реліз CERT-CLOSE-03 v0.82.18 доставлено у production. Live одноразове погашення одного серверно ізольованого QA-сертифіката підтверджено. UI precheck у формі бронювання залишається незакритим, бо потрібних елементів форми немає.
...
- У поточному index.html немає certCodeInput, certValidationResult і кнопки Validate. Тому booking form UI precheck не було як перевірити; опублікований пункт release notes про форму завищує фактичний стан. Це незакритий продуктний пункт, не закриття всієї картки.
...
## Післярелізний незакритий пункт
...
Водночас чинна форма бронювання не містить поля й кнопки перевірки сертифіката. Серверний validate API повертає правдиву заборону для QA-коду, але UI precheck у формі недоступний. Потрібен окремий вузький fix і реліз. Є також один нестабільний прогін certificate browser smoke на кандидатній гілці; точний release SHA пройшов 8/8 jobs.
