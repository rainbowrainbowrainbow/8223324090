...
const recent060ChangelogOrder = ['v0.60.6','v0.60.5','v0.60.4','v0.60.3','v0.60.2','v0.60.1','v0.60.0'];
...
checkPage('tasks.html', (doc, html) => {
    ...
    check('Tasks quick add keeps Details/Add controls stable when switching self/team', html.includes('grid-template-columns: minmax(260px, 1fr) minmax(160px, 210px) minmax(170px, 220px) auto auto') && html.includes('.task-assignee-select[hidden]') && html.includes('visibility: hidden') && tasksCode.includes("select.setAttribute('aria-hidden', 'true')") && tasksCode.includes('select.disabled = true'));
...
check('Sidebar removes duplicated day menu and keeps Additional as the only quick CRM section', !sidebarCode.includes('TODAY_MENU_HREFS') && !sidebarCode.includes('sidebar-today-menu-grid') && sidebarCode.includes('_removeSidebarTodayDock') && sidebarCode.includes("const EXTRA_MENU_HREFS = ['/dashboard', '/', '/tasks', '/chat']"));
...
check('Sidebar compact density reduces menu footprint without changing shell ownership', sidebarAuroraCss.includes('v0.55.30: compact sidebar density') && sidebarAuroraCss.includes('--eg-claude-sidebar-w: clamp(248px, 14vw, 300px)') && sidebarAuroraCss.includes('.sidebar-nav:not(.collapsed) .sidebar-alert-hero') && sidebarAuroraCss.includes('min-height: 42px !important') && sidebarAuroraCss.includes('.sidebar-nav:not(.collapsed) .sidebar-design-extra-link') && sidebarAuroraCss.includes('.sidebar-nav:not(.collapsed) .sidebar-links .nav-link'));
...
check('Sidebar v0.55.35 makes the upper command block compact without truncating profile text', sidebarAuroraCss.includes('v0.55.35: compact sidebar command deck') && sidebarAuroraCss.includes('--sb-command-compact-alert-min: 126px') && sidebarAuroraCss.includes('grid-template-columns: var(--sb-command-compact-avatar) minmax(0, 1fr) !important') && sidebarAuroraCss.includes('overflow-wrap: break-word !important') && sidebarAuroraCss.includes('.sidebar-alert-hero .sidebar-primary-action-label') && sidebarAuroraCss.includes('min-height: 38px !important'));
check('Sidebar alert hero counter is pageable instead of static', sidebarCode.includes('alertItems: []') && sidebarCode.includes('data-sidebar-alert-nav="prev"') && sidebarCode.includes('function _shiftSidebarAlertCursor') && sidebarCode.includes('function _buildSidebarAlertItems') && sidebarAuroraCss.includes('v0.55.37: alert hero carousel') && sidebarAuroraCss.includes('.sidebar-alert-nav-btn') && sidebarAuroraCss.includes('.sidebar-alert-hero-open'));
check('Sidebar alert hero opens the concrete current alert target', sidebarCode.includes('alertCurrentId') && sidebarCode.includes('alertTargetHref') && sidebarCode.includes('function _sidebarAlertTargetUrl') && sidebarCode.includes('function _openSidebarCurrentAlert') && sidebarCode.includes('function _navigateSidebarAlertTarget') && sidebarCode.includes('data-alert-id=') && sidebarCode.includes('openEl.dataset.alertId') && baseCss.includes('.ap-item.sidebar-alert-target'));
...
check('Sidebar enterprise redesign exposes component contracts, metric tones, ARIA and target widths', sidebarCode.includes('SIDEBAR_COMPONENTS') && sidebarCode.includes('SidebarShell') && sidebarCode.includes('UserSummaryCard') && sidebarCode.includes('function getMetricTone') && sidebarCode.includes("case 'tasks'") && sidebarCode.includes("case 'alerts'") && sidebarCode.includes("case 'leads'") && sidebarCode.includes('aria-expanded=') && sidebarCode.includes('aria-current="page"') && sidebarAuroraCss.includes('--eg-claude-sidebar-w: clamp(320px, 20vw, 340px)') && sidebarAuroraCss.includes('--eg-sidebar-collapsed-w: 80px') && sidebarAuroraCss.includes('width: min(92vw, 360px)'));
...
check('Shared assistant rail has premium voice lifecycle and blocked-playback fallback', assistantRailCode.includes('playbackRunId') && assistantRailCode.includes('function handlePlaybackFailure') && assistantRailCode.includes('voiceBlocked') && assistantRailCode.includes('dataset.playbackState') && assistantRailCode.includes('addToHistory: false') && assistantRailCode.includes('SPEECH_TTS_TIMEOUT_MS') && assistantRailCode.includes('function speakWithBrowserVoice') && assistantRailCode.includes('voice-loading'));
check('Shared assistant rail avoids bad browser Ukrainian speech fallback', assistantRailCode.includes('function normalizeBrowserSpeechText') && assistantRailCode.includes('function scoreBrowserSpeechVoice') && assistantRailCode.includes('speech_synthesis_no_ukrainian_voice') && assistantRailCode.includes('browser_has_no_ukrainian_voice') && assistantRailCode.includes("selectedVoice.lang || 'uk-UA'"));
...
check('HR exposes account center with safe deactivate controls', htmlContains('hr.html', 'data-tab="accounts"') && htmlContains('hr.html', 'accountCenterList') && hrCode.includes('function loadAccountCenter') && hrCode.includes('function deactivateKarinaAccounts') && htmlContains('routes/users.js', 'ACCOUNT_MANAGER_ROLES'));
