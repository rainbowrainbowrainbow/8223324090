...
checkPage('staff.html', (doc, html) => {
    ...
    const staffPulseTabletBlock = cssAtRuleBlock(staffPagesCss, '@media (max-width: 1120px)');
    ...
    check('Staff schedule keeps premium HR Pulse switcher and unified panel rhythm',
        ...
        && html.includes('js/staff-schedule-shell.js?v=0.78.8')
        && html.includes('js/hr-pulse-switcher.js?v=0.78.8')
...
check('Profile cabinet tasker has canonical My Day projection and default-collapsed composer', profileCode.includes('const CABINET_TASK_SEGMENTS') && profileCode.includes('function cabinetTaskMatchesSegment') && profileCode.includes('function cabinetSegmentCounts') && profileCode.includes('function renderCabinetTaskComposer') && profileCode.includes('function renderCabinetMyDayListModeToggle') && profileCode.includes('data-cabinet-composer-state=') && profileCode.includes("'expanded' : 'collapsed'") && profileCode.includes('data-cabinet-composer-toggle') && profileCode.includes('data-cabinet-composer-advanced') && profileCode.includes('function setCabinetTaskComposerExpanded') && profileCode.includes('function isCabinetPersonalTask') && profileCode.includes("visibility === 'me_only'") && profileCode.includes("category === 'improvement'") && profileCode.includes('function normalizeProfileTaskTab') && profileCode.includes("return tab === 'mytasks' ? 'myday' : tab;") && profileCode.includes('function syncProfileTabToUrl') && profileCode.includes('data-cabinet-due-preset') && profileCode.includes("value: 'month_end'") && profileCode.includes('data-cabinet-priority-preset') && profileCode.includes('data-cabinet-list-mode') && profileCode.includes('aria-label="Дата задачі:') && profileCode.includes('aria-label="Пріоритет:') && profileCode.includes('aria-label="Режим списку:') && !profileCode.includes('data-cabinet-due-preset="all"') && profilePagesCss.includes('.cabinet-task-composer.is-collapsed') && profilePagesCss.includes('[data-cabinet-composer-advanced][hidden]') && profilePagesCss.includes('.cabinet-task-priority') && profilePagesCss.includes('.cabinet-priority-chip') && profilePagesCss.includes('.cabinet-due-chip:focus-visible') && profilePagesCss.includes('.cabinet-list-mode-chip:focus-visible') && profilePagesCss.includes('text-overflow: ellipsis'));
check('Profile My Day command bar uses real segments instead of passive duplicated counters', profileCode.includes('const CABINET_MY_DAY_SEGMENTS') && profileCode.includes('function cabinetMyDaySegmentCounts') && profileCode.includes('function renderCabinetMyDaySegments') && profileCode.includes('function setCabinetMyDaySegment') && profileCode.includes('data-cabinet-my-day-segment') && profileCode.includes('data-cabinet-my-day-segment-panel') && profileCode.includes('renderCabinetMyDaySegments()') && !renderMyDayCommandCenterBody.includes('cabinet-day-command-stats') && profilePagesCss.includes('.cabinet-my-day-segments') && profilePagesCss.includes('.cabinet-my-day-segment:focus-visible'));
check('Profile My Day completed history sits full-width after composer without secondary duplication', renderMyDayCommandCenterBody.indexOf('renderCabinetTaskComposer') > -1 && renderMyDayCommandCenterBody.indexOf('renderCabinetCompletedHistoryStrip()') > renderMyDayCommandCenterBody.indexOf('renderCabinetTaskComposer') && renderMyDayCommandCenterBody.indexOf('cabinet-day-workspace') > renderMyDayCommandCenterBody.indexOf('renderCabinetCompletedHistoryStrip()') && !renderCabinetMyDaySecondaryBody.includes('renderCabinetCompletedHistoryStrip()') && profilePagesCss.includes('.cabinet-command-center > .cabinet-completed-strip') && profilePagesCss.includes('.cabinet-command-center > .cabinet-completed-strip .cabinet-completed-strip-summary') && profilePagesCss.includes('.cabinet-command-center > .cabinet-completed-strip .cabinet-completed-summary-action'));
...
check('Profile My Day compact task cards keep bounded metadata and one active checklist slice', profileCode.includes('let activeCabinetInlineTaskId') && profileCode.includes('function cabinetResolveActiveInlineTaskId') && profileCode.includes('function cabinetDefaultInlineTaskId') && profileCode.includes('function setCabinetActiveInlineTask') && profileCode.includes('function cabinetTaskVisibleBadges') && profileCode.includes('data-cabinet-visible-badge') && profileCode.includes('is-my-day-compact-card') && profileCode.includes('data-cabinet-active-subtask-slice') && profileCode.includes('function renderCabinetActiveSubtaskSlice') && profileCode.includes('aria-controls="cabinetSubtasksPanel') && profilePagesCss.includes('.cabinet-task-card.is-my-day-compact-card') && profilePagesCss.includes('.cabinet-task-visible-badge') && profilePagesCss.includes('.cabinet-subtask-active-slice') && profilePagesCss.includes('.cabinet-subtask-compact-summary') && profilePagesCss.includes('.cabinet-subtask-toggle::after'));
check('Profile My Day overdue segment renders a triage shell with existing delegated actions', profileCode.includes('function renderCabinetOverdueTriageList') && profileCode.includes('function renderCabinetOverdueTriageRow') && profileCode.includes('data-cabinet-overdue-triage') && profileCode.includes('data-cabinet-overdue-triage-row') && profileCode.includes('data-cabinet-task-action="move-to-today"') && profileCode.includes('data-cabinet-task-action="reschedule-overdue"') && profileCode.includes('data-reschedule-option="custom"') && profileCode.includes('data-source-surface="profile_my_cabinet_overdue_triage"') && profileCode.includes('data-cabinet-move-target="no_date"') && profileCode.includes('data-cabinet-move-method="triage"') && profileCode.includes("button.dataset.cabinetMoveMethod || 'menu'") && profilePagesCss.includes('.cabinet-overdue-triage') && profilePagesCss.includes('.cabinet-overdue-triage-row') && profilePagesCss.includes('.cabinet-overdue-triage-actions'));
check('Profile my tasks duplicate is neutralized into My Day plus canonical Tasks link', renderMyDayBody.includes('return renderMyDayCommandCenterTab();') && profileCode.includes('renderCabinetTaskComposer') && profileCode.includes('renderCabinetMyDaySegments()') && profileCode.includes('function renderCabinetMyDaySecondary') && profileCode.includes('renderCabinetPulseCluster()') && renderMyTasksBody.includes('return renderMyDayTab();') && profileCode.includes('function normalizeProfileTaskTab') && profileCode.includes("case 'mytasks': return renderMyDayTab();") && profileCode.includes('href="/tasks?view=today"') && profileCode.includes('href="/tasks?view=waiting"') && profileCode.includes('cabinet-command-center') && !profileCode.includes("setCabinetQuickMode('tasks')") && !profileCode.includes('href="/profile?tab=mytasks"') && !profileCode.includes('cabinet-shell--mytasks'));
check('Profile My Day sound controls live behind command-bar settings instead of a visible secondary panel', profileCode.includes('function renderCabinetMyDaySoundSettingsAction') && profileCode.includes('data-cabinet-my-day-sound-settings') && profileCode.includes('function openCabinetMyDaySoundSettings') && profileCode.includes("window.TaskUI?.openActionMenu?.(button") && profileCode.includes('function bindCabinetTaskSoundControls') && profileCode.includes('bindCabinetTaskSoundControls(root)') && profileCode.includes("apiPatch('/tasks/preferences'") && !profileCode.includes('<div class="cabinet-section-head"><h3>Звук</h3><span>налаштування</span></div>') && profilePagesCss.includes('.cabinet-day-action--settings') && profilePagesCss.includes('.cabinet-sound-settings-menu'));
...
check('Profile my day completed history strip uses compact details with real done payload and accessible day groups', profileCode.includes('renderCabinetCompletedHistoryStrip()') && profileCode.includes('function renderCabinetCompletedHistoryStrip') && profileCode.includes('cabinet-completed-strip--compact') && profileCode.includes('<details class="cabinet-completed-details">') && profileCode.includes('<summary class="cabinet-completed-strip-summary">') && profileCode.includes('cabinet-completed-summary-action') && profileCode.includes('function renderCabinetCompletedHistoryTile') && profileCode.includes('function groupCabinetCompletedHistoryByDay') && profileCode.includes('function renderCabinetCompletedDayDivider') && profileCode.includes('data-cabinet-completed-day-divider') && profileCode.includes('aria-describedby=') && profilePagesCss.includes('.cabinet-completed-strip-summary') && profilePagesCss.includes('.cabinet-completed-summary-action') && profilePagesCss.includes('.cabinet-completed-tile:focus-visible') && profilePagesCss.includes('.cabinet-completed-day-divider') && profilePagesCss.includes('.cabinet-completed-day-stats') && profilePagesCss.includes('.cabinet-completed-detail') && htmlContains('services/taskCabinetProjection.js', 'completedHistoryLimit') && htmlContains('services/taskCabinetProjection.js', "COALESCE(t.status, 'todo') = 'done'") && htmlContains('services/taskCabinetProjection.js', 'completedHistoryOverflow'));
...
check('Task quick widgets split completed-today and truthful open workload counts', profileCode.includes('function cabinetTaskQuickCounts') && profileCode.includes('виконано сьогодні') && profileCode.includes('cabinet-quick-split') && profileCode.includes('cabinet-quick-half--completed') && profileCode.includes('cabinet-quick-half--remaining') && profilePagesCss.includes('.cabinet-quick-divider') && sidebarCode.includes('/api/tasks/my-cabinet') && sidebarCode.includes('_isSidebarTaskCompletedToday') && sidebarCode.includes('_isSidebarTaskOpen') && sidebarCode.includes('sidebarOpenWorkload') && sidebarCode.includes('focusChipTasksDoneValue') && sidebarCode.includes('focus-chip-task-split') && sidebarAuroraCss.includes('.focus-chip-task-divider') && htmlContains('services/taskCabinetProjection.js', 'completed: quickStats.done_today') && htmlContains('services/taskCabinetProjection.js', 'openTaskCount') && htmlContains('services/taskCabinetProjection.js', 'open_count') && !htmlContains('services/taskCabinetProjection.js', 'const openTaskCount = rows.length') && !sidebarCode.includes("Number(tasks.assigned || 0) + Number(tasks.in_progress || 0)") && !sidebarCode.includes("activeCount = mine.filter(task => !['done', 'cancelled', 'archived'].includes(task.status) && _isSidebarTaskTodayOrUndated"));
...
check('Security CSP allowlists the bundled Microsoft Clarity snippet without opening broad script origins', securityMiddlewareCode.includes("script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://www.clarity.ms") && securityMiddlewareCode.includes("https://*.clarity.ms https://c.bing.com") && !securityMiddlewareCode.includes('script-src *'));
...
check('HR grouped IA keeps Pulse clean and vacancy workspace owns hiring surfaces',
    ...
    && htmlContains('hr.html', 'js/hr-pulse-switcher.js?v=0.78.8')
    ...
    && htmlContains('hr.html', 'js/staff-schedule-shell.js?v=0.78.8')
    && htmlContains('hr.html', 'js/staff-page.js?v=0.78.8')
...
const hrReportsHeroPanelRule = cssRuleText(hrPageCss, '.hr-reports-hero-panel');
const hrReportsHeroChipsRule = cssRuleText(hrPageCss, '.hr-reports-hero-chips');
const hrReportsChipRule = cssRuleText(hrPageCss, '.hr-reports-chip');
...
check('HR Pulse premium switcher keeps icon cards, routing, and accessible decorative affordances',
    ...
    && /display:\s*grid;/.test(hrReportsHeroPanelRule)
    && /display:\s*grid;/.test(hrReportsHeroChipsRule)
    && /display:\s*grid;/.test(hrReportsChipRule)
...
check('HR Pulse Today has CSS header metric chips, search, department segmentation, and compact live staff board',
    ...
    && /grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\);/.test(hrTodayHeroMetricsRule)
...
check('HR Pulse Reports has CSS header controls, summary chips, export, and report tables',
    ...
    && htmlContains('hr.html', 'class="hr-reports-hero-panel"')
    && htmlContains('hr.html', 'class="hr-reports-hero-chips"')
    ...
    && hrCode.includes("document.getElementById('reportExport')?.addEventListener('click', exportCSV)")
    && hrCode.includes('function reportRiskSummaryFromMetrics')
    ...
    && /display:\s*grid;/.test(hrReportsHeroPanelRule)
    && /display:\s*grid;/.test(hrReportsHeroChipsRule)
    && /grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\);/.test(hrReportsHeroChipsRule)
    && /border:\s*1px solid/.test(hrReportsChipRule)
    && /background:\s*linear-gradient/.test(hrReportsChipRule)
    && hrPageCss.includes('body.dark-mode .hr-reports-chip')
...
check('HR staff profile shows derived lifecycle checklist without a new schema',
    ...
    && hrRouteCode.includes('candidate_staff_link_missing')
