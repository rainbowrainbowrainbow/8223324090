...
async function buildWorkQueue({
    ...
    const todayTasks = await source(pool, warnings, 'tasks_today', () => loadTaskBucket(
        ...
        user,
        'today',
        "(t.scheduled_start_at IS NOT NULL AND (t.scheduled_start_at AT TIME ZONE 'Europe/Kyiv')::date = $1::date) OR (t.scheduled_start_at IS NULL AND t.deadline IS NOT NULL AND t.deadline >= NOW() AND t.deadline::date = $1::date) OR (t.scheduled_start_at IS NULL AND t.deadline IS NULL AND LEFT(COALESCE(t.date, ''), 10) = $1::text)",
        [todayStr],
        safeLimit,
        'task_due_today',
        businessScope
    ...
    const tomorrowTasks = await source(pool, warnings, 'tasks_tomorrow', () => loadTaskBucket(
        ...
        user,
        'tomorrow',
        "(t.scheduled_start_at IS NOT NULL AND (t.scheduled_start_at AT TIME ZONE 'Europe/Kyiv')::date = $1::date) OR (t.scheduled_start_at IS NULL AND t.deadline IS NOT NULL AND t.deadline::date = $1::date) OR (t.scheduled_start_at IS NULL AND t.deadline IS NULL AND LEFT(COALESCE(t.date, ''), 10) = $1::text)",
        [tomorrowStr],
        safeLimit,
        'task_due_tomorrow',
        businessScope
