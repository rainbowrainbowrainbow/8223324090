...
| `css/designer-guidebook.css` | designer | `page-scoped` | active | Design Board guidebook presentation, local theme states, tabs, examples, and downloadable template cards. |
...
| `css/pages-hr-foundation.css` | shared-ui | `shared` | active | HR employee-card foundation, lifecycle checklist, offboarding readiness, payroll hybrid config, role assignment, and dark-mode styles imported by `pages.css`. |
