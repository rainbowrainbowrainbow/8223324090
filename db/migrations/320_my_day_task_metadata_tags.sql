...
CREATE OR REPLACE FUNCTION my_day_valid_task_tags(values TEXT[])
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT values IS NOT NULL
       AND cardinality(values) <= 5
       AND NOT EXISTS (
           SELECT 1
           FROM unnest(values) AS tag
           WHERE tag IS NULL
              OR btrim(tag) = ''
              OR char_length(tag) > 32
       );
$$;
