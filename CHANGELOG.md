...
## v0.55.30 - Навігація: компактне меню

### Sidebar / Compact Density [codex]
- **Менша ширина** - shared sidebar зменшено до компактного width token, а сторінки продовжують брати offset з того самого shell contract.
- **Профіль без зайвої висоти** - картку користувача, avatar, статус, роль і summary ущільнено, але збережено повні імена та довгі посади без жорсткого обрізання.
- **Алерт-картка** - критичний блок став нижчим і щільнішим: менші badge, title, meta і кнопки без втрати читабельності.
- **Меню дня та додатково** - три швидкі кнопки, кастомні сторінки й основні nav rows отримали компактніші висоти, відступи та іконки.
- **Guardrail** - UI-smoke перевіряє compact density block, width token і ключові компактні стани меню.
