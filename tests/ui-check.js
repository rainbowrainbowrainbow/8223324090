...
check('Dashboard workspace uses grouped Ukrainian pro-style tool families', dashboardHtml.includes('board-pro-palette') && ['interaction', 'navigate', 'insert', 'draw', 'shape', 'connect', 'templates', 'actions'].every(family => dashboardHtml.includes(`data-board-tool-family="${family}"`)) && ['Режим', 'Навігація', 'Додати', 'Малювання', 'Фігури', 'Зв’язки', 'AI шаблони', 'Дії'].every(label => dashboardHtml.includes(`data-board-tool-family-label="${label}"`)) && dashboardCss.includes('content: attr(data-board-tool-family-label)') && dashboardCss.includes('.dashboard-btn[data-board-tool="rect"]::before'));
check('Dashboard workspace exposes Ukrainian tool labels, snap presets and clean module selection tokens', dashboardPageCode.includes('function setBoardSnapMode') && dashboardPageCode.includes('normalizeBoardSnapMode') && dashboardPageCode.includes('BOARD_SNAP_LABELS') && dashboardPageCode.includes('AI-помічник дошки') && dashboardHtml.includes('Віджети, дошка й малювання разом') && !dashboardHtml.includes('Workspace</button>') && !dashboardHtml.includes('Reset view') && dashboardCss.includes('--workspace-selection-ring') && dashboardCss.includes('.board-tool-snap-presets') && dashboardPageCode.includes('data-workspace-module="true"'));
