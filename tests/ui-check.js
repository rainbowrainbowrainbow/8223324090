...
checkPage('customers.html', (doc, html) => {
    ...
    check('Customer NPS tab keeps API/frontend contract and scoped review data aligned', customerPageCode.includes('data.avgNps ?? data.avgScore ?? data.avgRating') && customerPageCode.includes('data.recentReviews || data.recent') && customerPageCode.includes('function renderNpsExplainer(totalReviews)') && customerPageCode.includes('Чому може бути 0') && html.includes('data-tab="nps">NPS / відгуки') && html.includes('.nps-explainer') && htmlContains('routes/customers.js', "router.get('/nps-stats'") && htmlContains('routes/customers.js', 'const businessScope = ensureBusinessScope(req, res);') && htmlContains('routes/customers.js', 'avgNps: avgScore') && htmlContains('routes/customers.js', 'recentReviews') && htmlContains('routes/customers.js', "customerScopeCondition(avgParams, businessScope, 'er')"));
