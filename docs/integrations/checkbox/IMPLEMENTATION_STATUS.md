...
- Live production package baseline prepared for this handoff: `0.80.123` (`Gamification Schema Compatibility`).
- Release commit prepared for this handoff: final SHA to be confirmed after the v0.80.123 release commit.
...
- Release `0.80.123` is the package baseline prepared in this handoff. Reconfirm live version/commit before any future delivery or activation action.
...
- Mutation-free Checkbox test-mode readiness now reaches the real test cashier/register identity and remains blocked only because cash/card payment permissions are not explicitly enabled.
...
- Mutation-free test-mode readiness reached the real Checkbox test cashier/register identity on 2026-08-11 and failed closed because `cash_payment` and `card_payment` were not explicitly `true`. Enable both permissions for the test cashier in Checkbox before any controlled receipt test.

## Next Tasks

1. Enable or confirm `cash_payment=true` and `card_payment=true` for the Checkbox test cashier/register.
2. Re-run mutation-free readiness from local `CHECKBOX_SANDBOX_*`/`CHECKBOX_PARK_MIDDLE_TEST_*` secrets.
3. Run local config preflight from `CHECKBOX_PILOT_CONFIG_FILE` against a disposable/local PostgreSQL database before any apply. User `3` is the confirmed EventGenix CRM operator for test flow, while the Checkbox provider cashier remains the separate test cashier identity.
4. Run real Checkbox test-mode mutations only after a separate explicit approval, exact `is_test=true` readiness, and access-key requirements for the selected operations are confirmed.
5. Create a separate production activation task for real legal/tax mapping, secrets, webhook, and first controlled fiscal receipt.
...
- The current Checkbox test cashier already exposes `sales=true`; it must also expose `cash_payment=true` and `card_payment=true` before Phase 1 accepts money.
