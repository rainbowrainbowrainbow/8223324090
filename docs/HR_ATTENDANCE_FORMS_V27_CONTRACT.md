...
Версія контракту: `v27.2`

Дата фіксації: `2026-07-17`
...
У `v27.2` ці файли залишаються еталоном геометрії та пагінації. Місячна легенда, внутрішня зона денних клітинок і стандартні нижні колонтитули навмисно змінені цим контрактом.

## 2. Зафіксовані продуктові рішення

| Питання | Рішення v27 |
| --- | --- |
| Напис «Уход» чи «Вихід» | Preset `Еталон v27` зберігає еталонне слово **«Уход»** у заголовку, підписах і нижньому колонтитулі. «Вихід» може бути окремим майбутнім мовним preset, але не default v27. |
...
| Місячний табель | Активні денні клітинки залишаються повністю порожніми для ручного числа: `0,5`, `0,75` або `1`. Порожня клітинка означає відсутність запису; автоматичне заповнення місячних статусів не входить у цей контракт. |
...
| `monthlyInstruction` | Укажіть частку зміни числом: 0,5; 0,75 або 1 | 80 |
| `footerNote` daily | EG-FORMS-v27 • Лист приходу/уходу • A4 вертикально • CRM Event Genix | 120 |
| `footerNote` monthly | EG-FORMS-v27 • Місячний табель • A4 горизонтально • CRM Event Genix | 120 |
...
| Employee row | `7.3 mm` |
| Manual entry area | повна day cell, приблизно `7.35 × 7.3 mm` |

Сітка завжди містить 31 day column. Після 7, 14, 21 і 28 дня — посилений вертикальний separator. В активній клітинці немає внутрішнього квадрата чи штрихування. Легенда: `0,5 — половина зміни · 0,75 — три чверті зміни · 1 — повна зміна`.
...
### 11.1 Сумісність автоматизацій

- Нові автоматизації та нові job snapshots використовують renderer contract `v27.2`.
- У старій автоматизації точні попередні стандартні значення instruction/footer замінюються на поточні defaults лише на automation boundary.
- Порівняння виконується строго через exact match; часткові збіги та користувацькі тексти не змінюються.
- Глобальна нормалізація ручного request або вже збереженого job snapshot не переписує explicit legacy-тексти.
- Job зі статусом `queued` уже містить готовий PDF buffer і не перебудовується. Повторне надсилання такого job використовує ті самі PDF bytes.
- Незавершений legacy job без PDF bytes не рендериться новим renderer: він завершується з `HR_ATTENDANCE_JOB_TEMPLATE_VERSION_MISMATCH`. Новий v27.2 PDF створюється окремим запуском автоматизації з новим effective selection hash.
...
- `Уход` збережено як назву поля, стандартні нижні колонтитули локалізовані українською.
- Місячна легенда пояснює `0,5`, `0,75` і `1`; active day cells не містять внутрішніх квадратів або штрихування.
...
## 15. Межі контракту v27.2

Ця редакція змінює стандартні frontend-тексти, місячний PDF renderer і нормалізацію automation settings/job contract. Вона не змінює database schema, API contract, printer integration або правила читання фактичного attendance.

Implementation status (17.07.2026): generator, private HR API, PDF preview/download, Pulse UI та `queue_only` scheduler реалізовані. Фізичний printer/print-agent і автоматичне заповнення місячних клітинок attendance-даними залишаються окремими відкладеними етапами.
