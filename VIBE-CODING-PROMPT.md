...
2. [Шлях навчання для соло-розробника](#2-шлях-навчання)
3. [Базові принципи вайб-кодера](#3-базові-принципи-вайб-кодера)
4. [Покрокова система (від ідеї до продукту)](#4-покрокова-система)
5. [Як працювати з існуючим кодом](#5-як-працювати-з-існуючим-кодом)
6. [Типові помилки і як їх уникати](#6-типові-помилки)
7. [**ГОЛОВНИЙ ПРОМТ** (копіюй в Claude Code)](#7-головний-промт)
8. [Як користуватися промтом](#8-як-користуватися)
9. [Швидкі модифікатори](#9-модифікатори)
10. [Versioning](#10-versioning)
11. [Міні-шаблони запитів](#11-шаблони)
12. [Debugging для не-програміста](#12-debugging)
13. [Guardrails — що AI НЕ повинен робити](#13-guardrails)
14. [Чому Claude каже "ні" і як з цим працювати](#14-чому-claude-каже-ні)
15. [Quality Gates — контрольні точки](#15-quality-gates)
16. [Контекст-менеджмент між сесіями](#16-контекст-менеджмент)
17. [Deployment](#17-deployment)
18. [Робота з Git](#18-git)
19. [Приклади реальних сесій](#19-приклади)
20. [Фічі Claude Code для вайб-кодера](#20-фічі-claude-code)
21. [Вартість та вибір моделі](#21-вартість)
22. [5 анти-патернів (як НЕ треба)](#22-анти-патерни)
23. [Ресурси для навчання](#23-ресурси)
24. [Швидкий довідник (cheatsheet)](#24-cheatsheet)
...
## 2) Шлях навчання для соло-розробника

### Вайб-кодінг — це спектр

Andrej Karpathy (автор терміну) пояснив різницю:

| Рівень | Назва | Що робиш | Для чого |
|---|---|---|---|
| 1 | **Pure Vibe Coding** | Описуєш → AI генерує → приймаєш не читаючи | Прототипи, одноразові проекти, навчання |
| 2 | **Guided Vibe Coding** | Описуєш → AI генерує → переглядаєш → тестуєш | Особисті проекти, MVP |
| 3 | **Agentic Engineering** | Делегуєш → AI виконує → ти рев'юїш і контролюєш | Продукти для реальних користувачів |

**Ти зараз на рівні 1–2.** Цей документ допоможе тобі рухатися до рівня 3.

### "Вайб-такса" — чесне попередження

Дослідження Stack Overflow 2025: розробники з AI відчували себе на **20% швидшими**, але фактично витрачали **на 19% більше часу** на debugging і cleanup.

**Що це означає для тебе:**
- AI реально прискорює створення коду
- Але якщо ти не тестуєш і не рев'юїш — весь виграш з'їдає debugging
- Тому цей документ так наполягає на тестах і малих кроках

### Послідовність навчання (для абсолютного новачка)

**Тиждень 1–2: Основи**
1. Пройди [Prompt Engineering Interactive Tutorial](https://github.com/anthropics/courses/blob/master/prompt_engineering_interactive_tutorial/README.md) — безкоштовно, на GitHub
2. Поекспериментуй з Claude Code на простих задачах

**Тиждень 3–4: Практика**
3. Пройди курс [Claude Code in Action](https://anthropic.skilljar.com/claude-code-in-action) — офіційний, структурований
4. Зроби свій перший проект (використовуючи цей документ)

**Тиждень 5+: Поглиблення**
5. [Real-World Prompting](https://github.com/anthropics/courses/blob/master/real_world_prompting/README.md) — продвинуті патерни
6. [Tool Use Course](https://github.com/anthropics/courses/blob/master/tool_use/README.md) — як Claude використовує інструменти

### Поради для соло-розробника

1. **Ти — і розробник, і QA, і менеджер.** Тому Quality Gates (розділ 15) — обов'язкові. Немає кому перевірити за тебе.
2. **Git — твій рятівний круг.** Коміть часто. Коли немає команди — git це єдиний спосіб повернутися назад.
3. **Не намагайся зрозуміти ВСЕ.** Фокусуйся на поведінці (що програма робить), а не на коді (як вона це робить).
4. **Веди CONTEXT.md як щоденник проекту.** Завтра ти забудеш що робив сьогодні. AI теж забуде. CONTEXT.md — спільна пам'ять.
5. **Плануй сесії по 1–2 години.** Довші сесії = засмічений контекст = гірші відповіді. Краще 3 короткі сесії ніж одна на 6 годин.
6. **Writer/Reviewer Pattern:** Для важливих змін — зроби фічу в одній сесії, потім відкрий нову і попроси AI зробити code review з свіжим контекстом.

---

## 3) Базові принципи вайб-кодера
...
## 5) Як працювати з існуючим кодом
...
1. **Скопіюй** весь блок з розділу 7.
...
## 14) Чому Claude каже "ні" і як з цим працювати

### Ієрархія пріоритетів Claude

Claude має 4 пріоритети (від найвищого до найнижчого):

| # | Пріоритет | Що це означає |
|---|---|---|
| 1 | **Безпека** | Збереження людського контролю над AI |
| 2 | **Етика** | Чесність, уникнення шкоди |
| 3 | **Правила Anthropic** | Політики платформи і оператора |
| 4 | **Корисність** | Допомога тобі |

**Корисність — найнижчий пріоритет.** Коли Claude відмовляє або хеджує — це тому що пріоритети 1–3 перевищують пріоритет 4.

### Правило "1000 користувачів"

Коли Claude отримує запит, він уявляє: «Якщо 1000 різних людей надіслали це повідомлення — скільки з них мали б шкідливі наміри?»

**Що це означає для тебе:** Якщо Claude відмовляє на твій нормальний запит — **додай контекст**. Поясни чому ти це робиш і для чого.

### Типові відмови і як з ними працювати

| Що просиш | Чому відмовляє | Як переформулювати |
|---|---|---|
| Код для роботи з паролями | Може бути зловживання | «I'm building auth for my own booking app. Show me bcrypt hashing.» |
| Скрапінг даних | Приватність, IP | «I need to scrape my own website's public pages for migration.» |
| Код з "хакерськими" патернами | Безпека | «This is for a CTF challenge / security audit of my own system.» |
| Видалення файлів, rm -rf | Деструктивна дія | Розбий на кроки: спочатку покажи що видалиш, потім видали |

### Важливо: "Некорисність — не безпечна"

З офіційної конституції Anthropic:

> «Некорисність ніколи не є тривіально "безпечною". Ризики від занадто обережного Claude такі ж реальні як ризики від занадто шкідливого.»

**Що це означає:** якщо Claude відмовляє без причини — це баг, не фіча. Ти маєш право (ввічливо) наполягати і давати більше контексту.

### Характер Claude при кодуванні

1. **Помічає додаткові баги** — навіть якщо ти не просив. Це by design.
2. **Не "форсить" проходження тестів** — вирішує проблему, а не маскує.
3. **Поважає твій вибір технологій** — не буде пропонувати переписати React на Svelte без причини.
4. **Вчить за замовчуванням** — якщо хочеш тільки код без пояснень, скажи: «Code only, no explanation.»
5. **Чесний** — не каже "все добре" коли бачить проблему. Сприймай його зауваження як сигнал.
...
## 16) Контекст-менеджмент між сесіями
...
## 20) Фічі Claude Code для вайб-кодера
...
## 21) Вартість та вибір моделі

### Яку модель для чого (для соло-розробника)

| Модель | Ціна (за 1M токенів) | Коли використовувати | Аналогія |
|---|---|---|---|
| **Opus 4.6** | $5 вхід / $25 вихід | Складна архітектура, debugging, security | Старший інженер |
| **Sonnet 4.5** | $3 вхід / $15 вихід | Щоденне кодування, фічі, рефакторинг | Досвідчений розробник |
| **Haiku 4.5** | $1 вхід / $5 вихід | Прості задачі: форматування, перейменування | Швидкий помічник |

### Скільки коштує вайб-кодінг

| Що робиш | Приблизна вартість |
|---|---|
| Один складний промт | $0.01–0.05 |
| Сесія кодування (1–2 години) | $0.50–2.00 |
| Повний MVP за день | $5–15 |
| Місяць активної роботи | $50–150 |

**Claude Code Pro підписка** ($20/місяць) — найпростіший варіант для соло-розробника. Не треба рахувати токени.

### Як зекономити

| Стратегія | Економія | Складність |
|---|---|---|
| Використовуй Haiku для простих задач | 80% дешевше | Проста |
| Коротші сесії з `/clear` між задачами | Менше токенів на контекст | Проста |
| `CLAUDE.md` замість повторення промта | Кешування = -90% | Середня |
| `effort: low` для тривіальних задач | 40–70% менше токенів | Середня |

### Технічні характеристики Opus 4.6

| Характеристика | Значення | Що це означає для тебе |
|---|---|---|
| Контекстне вікно | 200K (стандарт), 1M (бета) | Можеш завантажити ~150,000 слів коду |
| Макс. вихід | 128K токенів | Може написати цілий модуль за один запит |
| Адаптивне мислення | Автоматичне | Сам вирішує скільки "думати" |
| SWE-bench | 80.8% | Розв'язує 4 з 5 реальних GitHub issues |
| Terminal-Bench | 65.4% (найвищий) | Найкращий в агентному кодуванні |
...
## 23) Ресурси для навчання

### Офіційні курси (безкоштовні)

| Курс | Посилання | Для чого |
|---|---|---|
| Prompt Engineering Tutorial | [GitHub](https://github.com/anthropics/courses/blob/master/prompt_engineering_interactive_tutorial/README.md) | Основи: як розмовляти з Claude ефективно |
| Claude Code in Action | [Skilljar](https://anthropic.skilljar.com/claude-code-in-action) | Практичне використання Claude Code |
| Real-World Prompting | [GitHub](https://github.com/anthropics/courses/blob/master/real_world_prompting/README.md) | Продвинуті патерни промптінгу |
| Tool Use Course | [GitHub](https://github.com/anthropics/courses/blob/master/tool_use/README.md) | Як Claude використовує інструменти |
| Claude API Course | [Skilljar](https://anthropic.skilljar.com/claude-with-the-anthropic-api) | Основи API (для розуміння як працює) |
| MCP Introduction | [Skilljar](https://anthropic.skilljar.com/introduction-to-model-context-protocol) | Підключення зовнішніх сервісів |

### Довідники і приклади коду

| Ресурс | Посилання | Для чого |
|---|---|---|
| Anthropic Cookbook | [GitHub](https://github.com/anthropics/anthropic-cookbook) | Готові рецепти для типових задач |
| Anthropic Quickstarts | [GitHub](https://github.com/anthropics/anthropic-quickstarts) | Стартові проекти (готові до запуску) |
| Agent Patterns | [GitHub](https://github.com/anthropics/anthropic-cookbook/tree/main/patterns/agents) | Як працюють агентні воркфлоу |
| Prompt Library | [Docs](https://platform.claude.com/docs/en/resources/prompt-library/library) | Готові оптимізовані промпти |
| Claude Code Docs | [Docs](https://code.claude.com/docs/en/overview) | Повна документація Claude Code |
| Prompting Best Practices | [Docs](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices) | Офіційні поради по промптінгу |

### Інструменти-альтернативи (для порівняння)

| Інструмент | Ціна | Найкраще для | Рейтинг |
|---|---|---|---|
| **Claude Code** | ~$20/міс | Серйозні проекти, великі кодбази | 4.5/5 |
| **Cursor** | Free/$20/міс | AI-native IDE, щоденне кодування | 4.9/5 |
| **GitHub Copilot** | Free (студенти)/$10/міс | Inline підказки, навчання | 4.2/5 |
| **Bolt.new** | Free/$20/міс | Швидкі прототипи в браузері | 4.3/5 |
| **Replit Agent** | Free/$25/міс | All-in-one без локальної установки | 4.1/5 |
| **Lovable** | Free/$25/міс | Дизайн-first, для не-кодерів | 4.4/5 |

**Порада:** Для серйозних проектів використовуй Claude Code. Для швидких прототипів — Bolt.new або Replit. Для щоденного кодування з IDE — Cursor.
...
```
┌──────────────────────────────────────────────────────────────┐
│            VIBE CODING v4.1 CHEATSHEET                        │
│            для соло-розробника-новачка                         │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  ПОЧИНАЄШ ПРОЕКТ:                                           │
│    1. Заповни 4 плейсхолдери (розділ 0)                      │
│    2. Встав промт (розділ 7) в CLAUDE.md або як повідомлення  │
│    3. Додай модифікатор (розділ 9) якщо треба                │
│    4. Відповідай на питання AI → один крок → тест            │
│    або: /init для автоматичного старту                        │
│                                                              │
│  ЩОСЬ ЗЛАМАЛОСЬ:                                            │
│    1. Скопіюй помилку дослівно                               │
│    2. Шаблон з розділу 12                                     │
...
│    Пройди Quality Gate (розділ 15)                            │
│                                                              │
│  AI ВІДМОВЛЯЄ:                                               │
│    Додай контекст. Поясни чому це легітимно (розділ 14)      │
│                                                              │
│  ЗБЕРЕГТИ ПРОГРЕС:                                           │
│    «Зроби коміт з описом що ми зробили»                      │
│                                                              │
...
│    /rename    → назвати сесію                                │
│                                                              │
│  ПРІОРИТЕТ НАВЧАННЯ:                                         │
│    Тиждень 1: Prompt Engineering Tutorial (GitHub)           │
│    Тиждень 2: Claude Code in Action (Skilljar)               │
│    Тиждень 3: Перший проект з цим документом                 │
│    Далі:     Real-World Prompting + Tool Use                 │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```
...
**Аудиторія: соло-розробник-новачок**
**Останнє оновлення: Лютий 2026**

**Джерела (всі перевірені):**
...
- [Replit: Best Vibe Coding Tools](https://replit.com/discover/best-vibe-coding-tools)
